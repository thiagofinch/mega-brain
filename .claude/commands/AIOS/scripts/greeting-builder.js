...
class GreetingBuilder {
  ...
  _analyzeModifiedFiles(modifiedFiles, currentStory) {
    ...
    const keyFiles = [];
    const patterns = [
      { regex: /greeting-builder\.js/, priority: 1, desc: 'do **`.aiox/scripts/greeting-builder.js`**', category: 'script' },
  ...
  _loadConfig() {
    try {
      const configPath = path.join(process.cwd(), '.aiox', 'core-config.yaml');
