...
- STEP 3: |
    ...
    1. Execute: node .aiox/scripts/generate-greeting.js {agent-id}
...
Add to `.aiox/scripts/agent-config-loader.js`:
...
async loadAgentDefinition(options = {}) {
  ...
  // Load from file
  const agentPath = path.join(process.cwd(), '.aiox', 'agents', `${this.agentId}.md`);
...
1. Add deprecation warning to `.aiox/scripts/config-loader.js`:
```javascript
/**
...
2. Check for usages:
```bash
grep -r "require.*config-loader" .aiox/
grep -r "from.*config-loader" .aiox/
```
...
1. `.aiox/scripts/batch-integrate-greeting-builder.js` ✅ (exists - verified)
2. `.aiox/scripts/apply-inline-greeting-all-agents.js` ❌ (not found - may already be deleted)
3. `.aiox/scripts/update-activation-instructions.js` ✅ (exists - verified)
4. `.aiox/scripts/batch-update-agents-session-context.js` ❌ (not found - may already be deleted)
...
1. **Verify script existence:**
   ```bash
   # Check each script
   test -f .aiox/scripts/batch-integrate-greeting-builder.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aiox/scripts/apply-inline-greeting-all-agents.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aiox/scripts/update-activation-instructions.js && echo "EXISTS" || echo "NOT FOUND"
   test -f .aiox/scripts/batch-update-agents-session-context.js && echo "EXISTS" || echo "NOT FOUND"
   ```

2. **Delete existing scripts only:**
   ```bash
   # Delete only if file exists (graceful handling)
   [ -f .aiox/scripts/batch-integrate-greeting-builder.js ] && rm .aiox/scripts/batch-integrate-greeting-builder.js
   [ -f .aiox/scripts/update-activation-instructions.js ] && rm .aiox/scripts/update-activation-instructions.js
   ```
...
Modify `.aiox/scripts/greeting-builder.js`:
...
Create `.aiox/scripts/generate-greeting.js`:
...
async function generateGreeting(agentId) {
  ...
  try {
    // Load core config
    const coreConfigPath = path.join(process.cwd(), '.aiox', 'core-config.yaml');
...
- STEP 3: |
    ...
    1. Execute: node .aiox/scripts/generate-greeting.js qa
    2. Capture the complete output
    3. Display the greeting exactly as returned
...
3. **Direct Script Test:**
   ...
   node .aiox/scripts/generate-greeting.js qa
...
4. **Error Handling Test:**
   ...
   node .aiox/scripts/generate-greeting.js invalid-agent
...
5. **Performance Test:**
   ...
   time node .aiox/scripts/generate-greeting.js qa
...
const AGENTS_DIR = path.join(process.cwd(), '.aiox', 'agents');
...
for agent in dev po sm pm architect analyst data-engineer devops aios-master ux-design-expert; do
  echo "Testing $agent..."
  node .aiox/scripts/generate-greeting.js $agent
  echo "---"
done
```
...
Create `.aiox/scripts/command-execution-hook.js`:
...
Create `.aiox/docs/session-update-pattern.md`:
...
const { generateGreeting } = require('../../.aiox/scripts/generate-greeting');
...
- `.aiox/scripts/generate-greeting.js` (Unified greeting generator)
- `.aiox/scripts/command-execution-hook.js` (Session update hook)
- `.aiox/docs/session-update-pattern.md` (Documentation)
- `tests/unit/generate-greeting.test.js` (Unit tests)
- `tests/integration/greeting-system-integration.test.js` (Integration tests)

### Files Modified

- `.aiox/scripts/agent-config-loader.js` (Expanded with `loadAgentDefinition()` and `loadComplete()`)
- `.aiox/scripts/greeting-builder.js` (Modified to accept pre-loaded context)
- `.aiox/scripts/config-loader.js` (Deprecated warning added)
- `.aiox/agents/qa.md` (Pilot - STEP 3 updated first)
- `.aiox/agents/*.md` (Remaining 10 agents - STEP 3 updated after QA validation)

### Files Deleted

- `.aiox/scripts/batch-integrate-greeting-builder.js` (Temporary migration script)
- `.aiox/scripts/apply-inline-greeting-all-agents.js` (Temporary migration script)
- `.aiox/scripts/update-activation-instructions.js` (Temporary migration script)
- `.aiox/scripts/batch-update-agents-session-context.js` (Temporary migration script)

### Files Referenced (No Changes)

- `.aiox/scripts/session-context-loader.js` (Used by generate-greeting.js)
- `.aiox/scripts/project-status-loader.js` (Used by generate-greeting.js)
- `.aiox/scripts/greeting-preference-manager.js` (Used by greeting-builder.js)
- `.aiox/scripts/context-detector.js` (Used by greeting-builder.js)
- `.aiox/scripts/workflow-navigator.js` (Used by greeting-builder.js)
