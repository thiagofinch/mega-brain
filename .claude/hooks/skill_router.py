...
# ---------------------------------------------------------------------------
# MCE-13.5: inline copy of engine.intelligence.utils.agent_files.find_agent_file
# Duplicated here because hooks run in isolated context without engine/ imports.
# ---------------------------------------------------------------------------
_LOWER_TO_UPPER_SR: dict[str, str] = {
    "agent.md": "AGENT.md",
    "soul.md": "SOUL.md",
    "memory.md": "MEMORY.md",
...
def _find_agent_file_sr(agent_dir: Path, name: str) -> Path | None:
    """Find agent file trying lowercase first, UPPERCASE fallback."""
    if not agent_dir.is_dir():
        return None
    lower = agent_dir / name.lower()
    if lower.exists():
        return lower
    upper_name = _LOWER_TO_UPPER_SR.get(name.lower())
    if upper_name:
...
AGENT_MEMORY_PATH = PROJECT_ROOT / ".data" / "agent_memory"
INDEX_PATH = PROJECT_ROOT / ".claude" / "mission-control" / "SKILL-INDEX.json"

# Cargo agents with persistent memory in .data/agent_memory/
...
def scan_skills() -> list[tuple[Path, str]]:
    ...
    # Scan sub-agents — MCE-13.5: use _find_agent_file_sr for case-insensitive lookup
    if SUBAGENTS_PATH.exists():
        for item in SUBAGENTS_PATH.iterdir():
            if item.is_dir() and not item.name.startswith("_"):
                if _find_agent_file_sr(item, "agent.md") is not None:
...
def extract_metadata(item_path: Path, item_type: str) -> dict:
    ...
    if item_type == "skill":
        ...
        # MCE-13.5: find agent.md case-insensitively
        md_file = _find_agent_file_sr(item_path, "agent.md")
    ...
    # Para sub-agents, verificar se tem soul.md / SOUL.md — MCE-13.5
    if item_type == "sub-agent":
        metadata["has_soul"] = _find_agent_file_sr(item_path, "soul.md") is not None
...
def get_subagent_context(subagent_path: str) -> str:
    ...
    # agent.md / AGENT.md (obrigatório) — MCE-13.5: case-insensitive lookup
    agent_md = _find_agent_file_sr(base_path, "agent.md")
    ...
    # soul.md / SOUL.md (opcional - personalidade) — MCE-13.5: case-insensitive lookup
    soul_md = _find_agent_file_sr(base_path, "soul.md")
...
def get_agent_memory(agent_name: str) -> str:
    """Carrega MEMORY.md compacto de .data/agent_memory/{agent}/."""
...
def main():
    ...
    Reads JSON from stdin, outputs JSON to stdout using
    the hookSpecificOutput schema (Claude Code current spec).
    """
    import sys
