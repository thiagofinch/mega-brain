...
2. PENDING.md - Pendências
3. CURRENT-TASK.md - Tarefa atual
4. JARVIS-DNA-PERSONALITY.md - DNA completo (inclui identity compact)
...
CRITICAL_FILES = {
    ...
    # memory_owner removed — JARVIS-MEMORY.md deprecated (Wave 3, 2026-04-08)
    # User preferences now live in auto-memory (~/.claude/projects/.../memory/)
    "pending": {"paths": [".claude/jarvis/PENDING.md"], "required": True, "max_age_hours": 48},
    "current_task": {
        "paths": [".claude/jarvis/CURRENT-TASK.md"],
        "required": False,
        "max_age_hours": 24,
    },
    "dna_personality": {
        "paths": [".claude/jarvis/JARVIS-DNA-PERSONALITY.md"],
        "required": True,
        "max_age_hours": 720,  # 30 dias
    },
    "soul": {
        "paths": ["system/02-JARVIS-SOUL.md"],
        "required": False,
        "max_age_hours": 720,
    },  # D2 Opção C — 2026-04-16
    # latest_session removed — was loading conversation snapshots into context.
    # Session recovery is handled by `claude --resume <UUID>` via SESSION-INDEX.json.
    "boot_sequence": {
        "paths": [".claude/jarvis/JARVIS-BOOT-SEQUENCE.md"],
        "required": True,
        "max_age_hours": 720,  # 30 dias
    },
}
...
def load_memory_owner() -> dict:
    ...
    NOTE (Wave 3, 2026-04-08): JARVIS-MEMORY.md was deprecated and removed
    from CRITICAL_FILES. User preferences now live in auto-memory
    (~/.claude/projects/.../memory/). This function preserves its signature
    for backwards compatibility but always returns an empty stub.
    """
    file_config = CRITICAL_FILES.get("memory_owner")
    ...
    filepath = find_file(file_config)
    if not filepath:
...
def load_soul() -> dict:
    ...
    if "### O Que JARVIS Faz Sem Pedir" in content:
        ...
        soul["autonomous_behaviors"] = [
            f"{b[0].strip()}: {b[1].strip()}" for b in behaviors if "Gatilho" not in b[0]
        ]
...
def load_identity_compact() -> dict:
    ...
    NOTE: identity_compact was merged into dna_personality and removed from
    CRITICAL_FILES. This function preserves its signature for backwards
    compatibility but always returns an empty stub.
    """
    file_config = CRITICAL_FILES.get("identity_compact")
    if file_config is None:
        return {
            "raw": "",
            "formula": "",
...
"""
    ...
    # load_latest_session removed — conversation snapshots should not be loaded into context.
    # Use `claude --resume <UUID>` for session recovery.
...
def check_system_integrity() -> dict:
    ...
    integrity = {"all_ok": True, "missing": [], "outdated": [], "loaded": [], "warnings": []}
    ...
    for name, config in CRITICAL_FILES.items():
        ...
        if filepath is None:
            ...
            if age["age_hours"] > config["max_age_hours"]:
                ...
                if config["required"]:
                    integrity["warnings"].append(f"{name} desatualizado há {age['age_days']} dias")
...
def format_task_box(current_task: dict | None) -> str:
    ...
    next_steps = current_task.get("next_steps") or ["Nenhum definido"]
    next_step = next_steps[0][:60]
