...
echo "📁 Handoffs found: ${#HANDOFFS[@]} (handoffs = human sessions, runners don't generate handoffs)"
echo ""

# Also count interactive vs runner commits for context
...
echo "   Signal: Co-Authored-By = human-in-loop. No Co-Authored-By = runner."
echo ""
...
for handoff in "${HANDOFFS[@]}"; do
  ...
  decisions=$(grep -niE '(decisão|decision|decided|escolhemos|optamos|migramos|criamos|extraímos|split|replaced|refactored|created|removed|added|chose|selected)' "$handoff" 2>/dev/null | head -15)
  decision_count=$(echo "$decisions" | grep -c '.' 2>/dev/null || echo 0)

  # Extract from DECISIONS MADE section if exists
  ...
  if [[ "$total" -gt 0 ]]; then
    ...
    # Save to temp file for pattern analysis
...
# Find decisions that appear in 2+ sessions
echo "  Decisions appearing in 2+ sessions (PATTERN CANDIDATES):"
echo ""

# Extract decision verbs + objects, normalize, count
cat "$ALL_DECISIONS_FILE" | \
...
if [[ "$MODE" == "--gaps-only" || "$MODE" == "full" ]]; then
  ...
  if [[ -f "$CARDS_FILE" ]]; then
    ...
    # Find decision themes NOT covered by heuristics
    ...
    for theme in $all_themes; do
      if ! echo "$heuristic_keywords" | grep -qi "$theme" 2>/dev/null; then
        ...
        if [[ "$count" -ge 3 ]]; then
          echo "    ⚠️  '$theme' appears ${count}x in decisions but has no heuristic"
          gaps=$((gaps + 1))
        fi
      fi
    done
    ...
    if [[ "$gaps" -eq 0 ]]; then
      echo "    ✅ No significant gaps found — existing heuristics cover major themes"
    fi
  else
    echo "  ⚠️  decision-cards.yaml not found. Cannot cross-reference."
  fi
fi
...
echo "  Run:  /extract-session-heuristics (after reviewing)"
echo "═══════════════════════════════════════════════════════════"
