...
# ─────────────────────────────────────────────────────────────────────────────
# DATA COLLECTOR INTEGRATION (graceful degradation)
# ─────────────────────────────────────────────────────────────────────────────
# S-08: Import centralized data collector for session aggregation and
# person name resolution. If unavailable (e.g. during isolated hook testing),
# falls back to existing inline JSONL parsing.

try:
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))  # reach project root
...
def _slug_card(slug: str, data: dict, orch_data: list[dict]) -> list[str]:
    ...
    # ── Resolve display name via data collector (or fallback) ─────────────
    if _HAS_COLLECTOR:
        try:
            display_name = resolve_person_name(slug)
        except Exception:
    ...
    lines = [
        _div(),
        _row(f"  {bucket_ic} SLUG: {display_name:<35s}  {bucket.upper()}"),
        _thin(),
    ...
    for step_num in sorted(completed):
        ...
        bool_vals = [
            v for k, v in vdict.items() if isinstance(v, bool) and not k.endswith("_count")
        ]
...
def _build_markdown_report(
    ...
    lines += _report_header(
        session_ts, len(by_slug), total_chunks, total_insights, total_agents, SESSION_HOURS
    )
    ...
    lines += [
        ...
        _row("  Salvado em           : logs/sessions/reports/"),
...
def main() -> None:
    try:
        ...
        session_ts = _now_iso()
        cutoff = datetime.now(UTC) - timedelta(hours=SESSION_HOURS)
        ...
        for slug in orch_by_slug:
            if slug and slug not in by_slug:
                by_slug[slug] = {
                    "steps": {},
                    "substeps": [],
                    "first_ts": orch_by_slug[slug][0].get("timestamp")
                    if orch_by_slug[slug]
                    else None,
                    "last_ts": orch_by_slug[slug][-1].get("timestamp")
                    if orch_by_slug[slug]
                    else None,
                    "bucket": "external",
                    "completed": [],
        ...
        # S-08: When data collector is available, use collect_session_data()
        # for enriched totals (filesystem-based agent/dossier counts, centralized
        # slug aggregation). Fall back to inline JSONL parsing otherwise.
        collector_data = None
        if _HAS_COLLECTOR:
            try:
                collector_data = collect_session_data(session_hours=float(SESSION_HOURS))
            except Exception:
                collector_data = None
        ...
        if collector_data is not None:
            # Use filesystem-based counts from data collector (more accurate)
            total_agents = collector_data.get("agents_count", 0)

            # Merge any slugs the collector found that our JSONL parsing missed
            for cslug in collector_data.get("slugs_processed", []):
                if cslug and cslug not in by_slug:
                    by_slug[cslug] = {
                        "steps": {},
                        "substeps": [],
                        "first_ts": None,
            ...
            # Fallback: count agents from step-10 substep entries
            total_agents = sum(
                sum(
                    1
                    for s in d.get("substeps", [])
        ...
        content = _build_markdown_report(
            session_ts, by_slug, orch_by_slug, total_chunks, total_insights, total_agents, cutoff
        )
