...
# Signal 2: Non-mechanical commits
...
# Signal 4: Handoff file in window
...
if [[ "$IS_REAL_SESSION" != true ]]; then
  echo "Not a substantive interactive session."
...
RUNNER_COMMITS=$(echo "$ALL_COMMITS" | grep -ciE '(chore\(outputs\)|score.card|baseline|metrics\.jsonl)' 2>/dev/null || echo 0)
...
if [[ "$MODE" == "--context" ]]; then
  if [[ "$ELIGIBLE" != true ]]; then
    echo "Session not eligible. Run without --context to see reasons."
    exit 1
  fi
