...
Architecture Rule Enforced:
  ...
  Agent memory lives at agents/{path}/MEMORY.md (resolved via ecosystem-registry.yaml)
...
ERROR HANDLING: fail-OPEN (2026-03-05 safety fix)
  - JSONDecodeError -> ALLOW (can't parse = not a valid hook call)
  - Any Exception -> ALLOW (don't block user operations on internal errors)
"""
...
def normalize_path(file_path: str):
    """Normalize path to project-relative, forward-slash format.

    Returns:
        str: relative path inside the project (e.g. ".claude/CLAUDE.md")
        None: file is OUTSIDE the project (out of scope for this hook —
              e.g. user's global ~/.claude/CLAUDE.md is not our concern)
    """
...
def main():
    try:
        ...
        # File is outside this project — out of scope for project hook.
        # User's global ~/.claude/CLAUDE.md, other projects, etc. are not
        # policed by this guard. (Fixes false-block on global config edits.)
            ...
            json.dumps(
                {
                    ...
                    "reason": (
                        ...
                        f"   Agent memory -> agents/{{path}}/MEMORY.md (via ecosystem-registry.yaml)"
                    ),
                }
            )
        )
