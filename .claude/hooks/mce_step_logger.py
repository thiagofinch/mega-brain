...
# Ícone de agente por categoria real (agents/{categoria}/)
AGENT_CATEGORY_ICON: dict[str, str] = {
    "external": "📚",
    "business": "🏢",
    "personal": "🧠",
    "cargo": "⚙️ ",
...
AGENT_BUCKET_LABEL: dict[str, str] = {
    "external": "📚 EXTERNAL",
    "business": "🏢 BUSINESS",
    "personal": "🧠 PERSONAL",
    "cargo": "⚙️  CARGO",
    "system": "🔧 SYSTEM",
    "business_master": "👑 MASTER",
}

# Ícone de fonte de ingestão
...
def _agent_icon(category: str) -> str:
    return AGENT_CATEGORY_ICON.get(category, "🤖")


def _agent_bucket_label(category: str) -> str:
    return AGENT_BUCKET_LABEL.get(category, "🤖 UNKNOWN")


def _detect_agent_category(agent_name: str, project_dir: Path) -> str:
    """Detecta categoria real do agente procurando em agents/."""
    agents_root = project_dir / "agents"
    categories = [
        ("business_master", "business-masters"),
        ("external", "external"),
        ("business", "business"),
        ("personal", "personal"),
        ("cargo", "cargo"),
    ...
    for cat, dirname in categories:
        cat_dir = agents_root / dirname
        if not cat_dir.exists():
            continue
        if (cat_dir / agent_name).exists():
            return cat
        for sub in cat_dir.iterdir():
            if sub.is_dir() and sub.name.lower() == agent_name.lower():
                return cat
    return "business"
...
def _bar(value: int, max_val: int, width: int = 18, filled: str = "█", empty: str = "░") -> str:
...
def _tone_bar(score: float, width: int = 10, filled: str = "●", empty: str = "○") -> str:
...
def _get_total_steps() -> int:
    """Total de steps derivado de STEP_DISPLAY — nunca hardcoded.
    Adicionar/remover step em STEP_DISPLAY atualiza automaticamente."""
    return max(STEP_DISPLAY.keys()) if STEP_DISPLAY else 12


def _progress_section(current_step: int) -> list[str]:
    """Pipeline progress bar dinâmica — total derivado de STEP_DISPLAY."""
    ...
    row1 = "  ".join(sym(s) for s in range(half + 1))
    row2 = "  ".join(sym(s) for s in range(half + 1, total + 1))
    pct = round(current_step / total * 100) if total else 0
    bar_done = round(current_step / total * 30) if total else 0
    prog_bar = "█" * bar_done + "░" * (30 - bar_done)

    return [
        _thin(),
        _section(f"PIPELINE PROGRESS  {current_step}/{total} steps  [{pct:>3d}%]"),
        _row(f"  {prog_bar}"),
        _row(f"  {row1}"),
        _row(f"  {row2}"),
        _bottom(),
...
def _panel_step5(slug: str, bucket: str, m: dict, v: dict) -> str:
    ...
    for tag, label in DNA_TAGS:
        ...
        lines.append(_row(f"  {label} {_bar(count, max_tag)} {count:>4d} ({pct:>2d}%)"))
...
def _panel_step6(slug: str, bucket: str, m: dict, v: dict) -> str:
    ...
    max_t = max(by_type.values(), default=max(m.get("total_patterns", 1), 1))
...
def _panel_step7(slug: str, bucket: str, m: dict, v: dict) -> str:
    ...
    lines += [
        ...
        _row(
            f"  Total Values  : {m.get('values_count', 0):>4d}"
...
# ─────────────────────────────────────────────────────────────────────────────
# BATCH PANEL — display consolidado multi-slug para modo paralelo
# Mesmo padrão visual do logger individual, mas agrega N slugs por fase.
# ─────────────────────────────────────────────────────────────────────────────


def _read_events_jsonl(project_dir: Path) -> list[dict]:
    path = project_dir / ".data" / "pipeline-events.jsonl"
    if not path.exists():
        return []
    events: list[dict] = []
    try:
...
def _read_batch_status(project_dir: Path) -> dict:
    path = project_dir / ".data" / "BATCH-STATUS.json"
    if not path.exists():
        return {}
    try:
...
def build_batch_panel(
    ...
    """Painel consolidado no padrão visual do logger.
    Agrupa por batch_id — múltiplas ingestões simultâneas em seções separadas.
    Steps e total derivados de STEP_DISPLAY — nunca hardcoded.
    Agentes com ícone de categoria e label de bucket real.
    """
    events = _read_events_jsonl(project_dir)
    batch = _read_batch_status(project_dir)
    total_steps = _get_total_steps()

    total_files = batch.get("total", 0)
    processed = batch.get("processed", 0)
    eta = batch.get("eta", "--:--")
    active_workers = batch.get("active_workers", 0)

    # Agregar por batch_id
    batches: dict[str, dict] = {}
    for ev in events:
        bid = ev.get("batch_id", "default")
        if bid not in batches:
            batches[bid] = {
                "source": ev.get("source", "inbox"),
                "phase_done": {},
                "phase_active": {},
                "insights_high": [],
                "entities_new": [],
        ...
        if t == "phase_complete":
            b["phase_done"][phase] = b["phase_done"].get(phase, 0) + 1
        elif t == "phase_start":
            b["phase_active"].setdefault(phase, [])
            if slug not in b["phase_active"][phase]:
                b["phase_active"][phase].append(slug)
        elif t == "insight" and ev.get("priority") in ("HIGH", "CRITICAL"):
            b["insights_high"].append(ev)
        elif t == "entity_created":
            b["entities_new"].append(ev)
        elif t in ("dossier_created", "dossier_updated"):
            b["dossiers"].append(ev)
        elif t == "agent_created":
            ev["_category"] = _detect_agent_category(ev.get("agent_name", ""), project_dir)
            b["agents_new"].append(ev)
        elif t == "agent_queued_l3":
            ev["_category"] = _detect_agent_category(ev.get("agent_name", ""), project_dir)
            b["agents_l3"].append(ev)
        elif t == "dna_extracted":
            b["dna_items"].append(ev)
        elif t == "trigger_fired":
            b["triggers"].append(ev)

    # Fonte única ou múltipla
    if len(batches) == 1:
        only = next(iter(batches.values()))
        phase_done = only["phase_done"]
        phase_active = only["phase_active"]
        insights_high = only["insights_high"]
        entities_new = only["entities_new"]
        dossiers = only["dossiers"]
        agents_new = only["agents_new"]
        agents_l3 = only["agents_l3"]
        dna_items = only["dna_items"]
        triggers = only["triggers"]
        src = only["source"]
        src_icon = SOURCE_ICON.get(src, "📥")
        header_source = f"  🏭 MCE PIPELINE  {src_icon} {src.upper():<12s}  {active_workers}W  ⏱ {datetime.now().strftime('%H:%M:%S')}"
    else:
        phase_done: dict[int, int] = {}
        phase_active: dict[int, list] = {}
        insights_high: list = []
        entities_new: list = []
        dossiers: list = []
        agents_new: list = []
        ...
        for b in batches.values():
            for ph, n in b["phase_done"].items():
                phase_done[ph] = phase_done.get(ph, 0) + n
            for ph, slugs in b["phase_active"].items():
                phase_active.setdefault(ph, [])
                phase_active[ph].extend(slugs)
            insights_high.extend(b["insights_high"])
            entities_new.extend(b["entities_new"])
            dossiers.extend(b["dossiers"])
            agents_new.extend(b["agents_new"])
            agents_l3.extend(b["agents_l3"])
            dna_items.extend(b["dna_items"])
            triggers.extend(b["triggers"])
        sources_str = "  ".join(
            f"{SOURCE_ICON.get(bv['source'], '📥')}{bv['source'].upper()[:6]}"
            for bv in batches.values()
        )
        header_source = f"  🏭 MCE PIPELINE  MULTI  {sources_str}  {active_workers}W  ⏱ {datetime.now().strftime('%H:%M:%S')}"

    ts = datetime.now().strftime("%H:%M:%S")  # noqa: F841 — kept for potential use

    lines: list[str] = [_top(), _row(header_source), _div()]

    # Progresso geral
    if total_files > 0:
        pct = round(processed / total_files * 100)
        bar_done = round(processed / total_files * 30)
        prog = "█" * bar_done + "░" * (30 - bar_done)
        lines += [
            _section(f"PROGRESSO  {processed}/{total_files} arquivos  [{pct:>3d}%]  ETA {eta}"),
            _row(f"  {prog}"),
        ]

    # Fases
    lines += [_div(), _section("FASES — TODOS OS SLUGS")]
    for step in range(1, total_steps + 1):
        done = phase_done.get(step, 0)
        active = phase_active.get(step, [])
        name = STEP_DISPLAY.get(step, f"S{step:02d}")
        bar = _bar(done, max(total_files, 1), width=16)
        active_str = f"  ↻ {','.join(active[:3])}" if active else ""
        sym = "◉" if step == current_step else ("●" if done > 0 else "○")
        lines.append(_row(f"  S{step:02d}{sym} {name:<12s}  {bar}  {done:>3d}✓{active_str}"))

    # Eventos semânticos
    has_events = any(
        [insights_high, entities_new, dossiers, agents_new, agents_l3, dna_items, triggers]
    )
    if has_events:
        lines += [_div(), _section("EVENTOS RECENTES")]

    if insights_high:
        lines += [_thin(), _section("⚡ INSIGHTS CRÍTICOS / HIGH")]
        for ev in insights_high[-4:]:
            p_icon = "🚨" if ev.get("priority") == "CRITICAL" else "🔴"
            text = ev.get("text", "")[: PW - 14]
            lines.append(_row(f"  {p_icon} [{ev.get('slug', '?')}]  {text}"))
            if ev.get("impact"):
                lines.append(_row(f"       → {ev['impact'][: PW - 9]}"))

    if entities_new:
        lines += [_thin(), _section("🔗 ENTIDADES CRIADAS / RESOLVIDAS")]
        for ev in entities_new[-4:]:
            linked = ev.get("linked_to", [])
            linked_str = f" → {', '.join(linked[:2])}" if linked else ""
            lines.append(_row(f"  • {ev.get('name', '')}{linked_str}"[: PW + 2]))

    if dossiers:
        lines += [_thin(), _section("📁 DOSSIERS")]
        for ev in dossiers[-3:]:
            verb = "CRIADO" if ev.get("type") == "dossier_created" else "UPDATE"
            fn = ev.get("filename", "")
            n_i = ev.get("insights", ev.get("insights_added", 0))
            n_s = ev.get("sections", ev.get("sections_added", 0))
            detail = f"+{n_i}ins" + (f" +{n_s}sec" if n_s else "")
            lines.append(_row(f"  {verb}  {fn[: PW - 14]}  {detail}"))

    if agents_new or agents_l3:
        lines += [_thin(), _section("🤖 AGENTES")]
        for ev in agents_new[-3:]:
            cat = ev.get("_category", "business")
            icon = _agent_icon(cat)
            bucket = _agent_bucket_label(cat)
            name = ev.get("agent_name", "")
            lines.append(_row(f"  ✅ {icon} {name:<25s}  {bucket}"))
            if ev.get("reason"):
                lines.append(_row(f"     └ {ev['reason'][: PW - 7]}"))
        for ev in agents_l3[-2:]:
            cat = ev.get("_category", "business")
            icon = _agent_icon(cat)
            bucket = _agent_bucket_label(cat)
            name = ev.get("agent_name", "")
            lines.append(_row(f"  ⏳ {icon} {name:<25s}  {bucket}  ← L3 QUEUE"))

    if dna_items:
        lines += [_thin(), _section("🧬 DNA EXTRAÍDO")]
        for ev in dna_items[-3:]:
            layers = ev.get("layers", {})
            layers_str = "  ".join(f"{k[:3]}+{v}" for k, v in layers.items())
            lines.append(_row(f"  [{ev.get('person', '?')}]  {layers_str[: PW - 10]}"))

    if triggers:
        lines += [_thin(), _section("⚡ TRIGGERS ATIVOS")]
        for ev in triggers[-3:]:
            detail = f"  {ev.get('detail', '')[:30]}" if ev.get("detail") else ""
            lines.append(_row(f"  • {ev.get('trigger', '')}{detail}"))

    lines += _progress_section(current_step)
    return "\n".join(lines)
...
def _panel_fallback(step: int, step_name: str, slug: str, rel: str) -> str:
    lines = [
        _top(),
        _row(
            f"  STEP {step:02d} — {STEP_DISPLAY.get(step, step_name.upper()):<20s}  [ {_fmt_slug(slug)} ]"
        ),
        _div(),
...
def _build_panel(
    step: int, step_name: str, slug: str, rel: str, bucket: str, m: dict, v: dict
) -> str:
    dispatch = {
        3: _panel_step3,
        4: _panel_step4,
...
def _slug_from_path(rel: str) -> str:
    """Extract person/source slug from a pipeline file path.

    Priority chain:
        1. Segment after ``persons/`` (highest priority)
        2. Segment after bucket markers (``external``, ``business``, ``personal``)
           walking forward past SKIP_SEGMENTS
        3. Segment after ``agents/`` walking past SKIP_SEGMENTS
        4. Empty string (caller falls through to ``_slug_from_state()``)

    SYNC: This logic must match chronicler_data_collector.resolve_slug_from_path()
    """
    # SYNC: SKIP_SEGMENTS must match chronicler_data_collector.SKIP_SEGMENTS
    skip_segments = {
        "dna",
        "dossiers",
        "sources",
    ...
    for i, part in enumerate(parts):
        if part in ("external", "business", "personal"):
            for j in range(i + 1, len(parts)):
                cand = parts[j]
                if "." in cand:
                    break  # Hit a filename, stop walking
                if cand not in skip_segments:
                    return cand
    ...
    if isinstance(persons, dict):
        for _name, pdata in persons.items():
            ins_list = (
                ...
                else (pdata if isinstance(pdata, list) else [])
            )
...
def main() -> None:
    ...
    try:
        ...
        # ── Build ASCII panel ─────────────────────────────────────────────
        # Modo batch (paralelo): painel consolidado multi-slug
        # Modo serial: painel individual por arquivo
        events_path = PROJECT_DIR / ".data" / "pipeline-events.jsonl"
        if events_path.exists():
            try:
                panel = build_batch_panel(PROJECT_DIR, current_step=step, current_slug=slug)
            except Exception:
