...
# ---------------------------------------------------------------------------
_LOWER_TO_UPPER_AIU: dict[str, str] = {
    "agent.md": "AGENT.md",
    "soul.md": "SOUL.md",
    "memory.md": "MEMORY.md",
...
def _find_agent_file_aiu(agent_dir: Path, name: str) -> Path | None:
    ...
    if lower.exists():
        return lower
    upper_name = _LOWER_TO_UPPER_AIU.get(name.lower())
    if upper_name:
...
UPDATE_LOG = BASE_DIR / ".data" / "logs" / "agent-index-updates.jsonl"
...
def is_agent_file(file_path: str) -> bool:
    ...
    MCE-13.5: accepts both lowercase and UPPERCASE filenames.
    """
    path = Path(file_path)
    # Normalise to lowercase for comparison
    canonical_names = {"agent.md", "soul.md", "memory.md"}
    return (
        "agents/" in file_path
        and path.name.lower() in canonical_names
...
def scan_agents() -> dict:
    ...
    # Scan external (expert mind clones) — MCE-13.5: case-insensitive lookup
    minds_dir = AGENTS_DIR / "external"
    if minds_dir.exists():
        for agent_dir in minds_dir.iterdir():
            if agent_dir.is_dir() and not agent_dir.name.startswith("_"):
                if _find_agent_file_aiu(agent_dir, "agent.md") is not None:
                    agents["minds"].append(
                        {
                            "id": agent_dir.name,
                            "path": str(agent_dir.relative_to(BASE_DIR)),
                            "has_soul": _find_agent_file_aiu(agent_dir, "soul.md") is not None,
                            "has_memory": _find_agent_file_aiu(agent_dir, "memory.md") is not None,
                        }
                    )

    # Scan conclave — MCE-13.5: case-insensitive lookup
    ...
    # Scan cargo (grouped) — MCE-13.5: case-insensitive lookup
