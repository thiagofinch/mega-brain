...
ALTERNATIVES = {
    "docs": "reference/  (or update engine/paths.py if new category needed)",
}
...
def main():
    ...
    path = Path(file_path)
    try:
        # Project root = .claude/hooks/ -> ../.. (3 levels up)
    ...
    if top_dir in PROHIBITED_DIRS:
        alt = ALTERNATIVES.get(top_dir, "Check engine/paths.py for correct location")
