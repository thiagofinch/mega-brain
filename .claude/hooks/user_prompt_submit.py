...
def detect_special_intents(prompt):
    ...
    if any(
        ...
        for phrase in ["war room", "debate sobre", "múltiplas perspectivas", "prós e contras"]
    ):
        intents.append("war_room_request")

    if any(
        phrase in prompt_lower
        for phrase in ["use os agentes", "consulte especialistas", "análise profunda"]
    ):
...
def main():
    ...
    using the hookSpecificOutput schema (Claude Code current spec)."""
    try:
        ...
        # Output: only emit if we have context to inject
        if context_parts:
            print(
                json.dumps(
                    {
