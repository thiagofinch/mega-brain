...
ECOSYSTEM_REGISTRY = Path(PROJECT_DIR) / "agents" / "_registry" / "ecosystem-registry.yaml"
...
def cascade_file_canonical(
    ...
    """
    Canonical cascade dispatcher (C-17-CASCADE-DISPATCHER fix, 2026-05-12).

    Takes a destination from the canonical format (path-style filename) and
    resolves to actual file path using the validator's multi-bucket resolvers.
    Appends a "Cascading update — BATCH-XXX" footer to the resolved file.

    Args:
        section: One of "agents", "dnas", "dossiers", "playbooks"
        destination_name: The destination filename from the batch
            (e.g. "acme/MEMORY.md", "dossier-acme.md", "voice-dna.yaml")
        batch_id: The batch ID to append as reference
        metadata: Batch metadata dict

    Returns:
        Action dict: {action, destination, path, success}
    """
    import sys

    scripts_dir = str(Path(PROJECT_DIR) / ".claude" / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)

    try:
        from validate_cascading_integrity import (
            resolve_agent_path,
            resolve_dna_path,
        ...
        return {
            "action": f"{section}_resolver_unavailable",
    ...
    if not resolver:
        return {
            "action": "unknown_section",
            "destination": destination_name,
            "section": section,
            "success": False,
        }

    file_path = resolver(destination_name)
    if file_path is None or not file_path.exists():
        return {
            "action": f"{section[:-1]}_not_found",
            "destination": destination_name,
            "section": section,
            "success": False,
        }

    # Idempotency: if file already references this batch, skip
    try:
    ...
    if batch_id in existing:
        return {
            "action": f"{section[:-1]}_already_referenced",
            "destination": destination_name,
            "path": str(file_path),
            "success": True,
        }

    # Compose footer in the file's native format (yaml vs md)
    ...
    if file_path.suffix in (".yaml", ".yml"):
        footer = (
            f"\n\n# ---\n"
            f"# Cascading update — {batch_id}\n"
            f"# Date: {ts}\n"
            f"# Source: {source}\n"
            f"# (auto-appended by post_batch_cascading.py)\n"
        )
    elif file_path.suffix == ".json":
        # JSON files can't have raw comments — skip cascade write but report
        # idempotency to avoid blocking validation. The DNA-CONFIG mechanism
        # handles JSON state separately.
        return {
            "action": f"{section[:-1]}_json_skipped",
            "destination": destination_name,
            "path": str(file_path),
            "note": "JSON files require structured updates, not footer appends",
            "success": True,
        }
    else:
        # markdown / other text — append a small footer section
        footer = (
            f"\n\n---\n\n"
            f"## Cascading update — `{batch_id}`\n\n"
            f"**Date:** {ts}  \n"
            f"**Source:** {source}  \n"
            f"_Auto-appended by post_batch_cascading.py (canonical dispatcher)._\n"
        )
    ...
    try:
        with open(file_path, "a", encoding="utf-8") as f:
            f.write(footer)
        return {
            "action": f"{section[:-1]}_referenced",
            "destination": destination_name,
            "path": str(file_path),
            "success": True,
        }
...
def extract_destinations_canonical(batch_content: str) -> dict[str, list[dict]]:
    """
    Extrai destinos do formato CANÔNICO pós-C-17 (batch_auto_creator.py).

    Format esperado:
        🤖 AGENTES A ALIMENTAR
           • acme/MEMORY.md
           • acme/DNA-CONFIG.yaml
        ...
        🧬 DNAs ENRIQUECIDOS
           • acme/voice-dna.yaml
           • acme/behavioral-patterns.yaml

        📁 DOSSIERS
           • dossier-acme.md

    C-17-CASCADE-PARSER fix (2026-05-12): cascade hook parser estava preso no
    formato legacy (`AGENTES A ALIMENTAR\\n   NAME-IN-CAPS`). C-17 atualizou
    template + validator regex mas hook parser ficou desincronizado.
    Resultado: parser retornava {}, cascade reportava sucesso vazio (silent
    no-op em 4/5 batches recentes).

    Retorna mesmo schema que extract_destinations (legacy):
        {"agents": [{"name": ..., "frameworks": []}], "playbooks": [...],
         "dnas": [...], "dossiers": [...]}
    """
    destinations = {"agents": [], "playbooks": [], "dnas": [], "dossiers": []}

    # Markers canônicos (emoji + label) → section type
    sections = {
        "agents": ["AGENTES A ALIMENTAR", "🤖"],
        "playbooks": ["PLAYBOOKS IMPACTADOS", "📘"],
        "dnas": ["DNAs ENRIQUECIDOS", "DNA ENRIQUECIDO", "🧬"],
        "dossiers": ["DOSSIERS", "📁"],
    }

    # Item regex: matches `• {filename}` or `· {filename}`
    item_pattern = re.compile(
        r"^\s*[•·]\s*([A-Za-z][\w./-]*\.(?:md|yaml|json))",
        re.MULTILINE,
    )

    # Compute section boundaries by NEXT marker (same logic as validator C-17)
    all_marker_idxs = []
    for dest_type, markers in sections.items():
        for marker in markers:
            idx = batch_content.find(marker)
            if idx != -1:
    ...
    for i, (start_idx, dest_type, marker) in enumerate(all_marker_idxs):
        next_start = (
            all_marker_idxs[i + 1][0] if i + 1 < len(all_marker_idxs) else len(batch_content)
        )
        section_text = batch_content[start_idx:next_start]
        ...
        for m in item_pattern.finditer(section_text):
            ...
            if not filename:
                continue
            # Skip duplicates
            existing = {d["name"] for d in destinations[dest_type]}
            if filename in existing:
                continue
            destinations[dest_type].append({"name": filename, "frameworks": []})
...
def extract_destinations(batch_content: str) -> dict[str, list[dict]]:
    ...
    C-17-CASCADE-PARSER (2026-05-12): tenta parser canônico primeiro (formato
    pós-C-17 com path-style refs). Fall back para parser legacy se canônico
    retornar vazio (mantém backward compat com batches antigos).

    Suporta multiplos formatos de batch incluindo:
    - Formato canônico (post-C-17): • acme/MEMORY.md
    ...
    """
    # C-17-CASCADE-PARSER: try canonical parser FIRST (post-C-17 format).
    # If it yields any destinations, return immediately. Otherwise fall through
    # to legacy parser for backward compat with old batches.
    canonical = extract_destinations_canonical(batch_content)
    if sum(len(v) for v in canonical.values()) > 0:
        return canonical
    ...
    # C-17-CASCADE-PARSER (2026-05-12): if legacy parser yielded nothing,
    # try canonical parser (post-C-17 format with path-style refs).
    legacy_total = sum(len(v) for v in destinations.values())
    if legacy_total == 0:
        canonical = extract_destinations_canonical(batch_content)
...
@lru_cache(maxsize=1)
def _load_categories_from_registry() -> tuple[str, ...]:
    """Load the top-level agent categories from the ecosystem registry.

    The ecosystem registry (``agents/_registry/ecosystem-registry.yaml``)
    is the single source of truth for which agent categories exist in this
    repo. Each entry has a ``category`` field shaped like
    ``external/cargo/sales`` or ``business/partners`` — the FIRST segment
    is the top-level on-disk category (``external``, ``business``, etc.).

    Returns:
        Tuple of top-level category slugs in registry-declared order,
        de-duplicated. Tuple (immutable) so ``lru_cache`` can safely
        return the same instance to all callers.

    Fail-open:
        On any load error (missing file, malformed YAML, no agents key)
        returns the empty tuple. ``find_agent_path`` falls back to
        scanning every direct child of ``agents/`` (excluding ``_*``).
    """
    try:
        if not ECOSYSTEM_REGISTRY.exists():
            return ()
        with open(ECOSYSTEM_REGISTRY, encoding="utf-8") as handle:
        ...
        # Any failure → empty tuple; caller falls back to disk scan.
        return ()


def _categories_from_disk() -> tuple[str, ...]:
    """Disk fallback: every direct child of agents/ (excluding _* names).

    Used when the registry is unavailable or empty. Never raises.
    """
...
def find_agent_path(agent_name: str) -> Path | None:
    ...
    N2/G6 (2026-05-13): Categories are now loaded dynamically from
    ``agents/_registry/ecosystem-registry.yaml`` via
    ``_load_categories_from_registry`` (cached with ``lru_cache``). If the
    registry is missing/malformed the search falls back to scanning every
    direct child of ``agents/`` (excluding ``_*`` slugs). The previous
    hardcoded REAL_CATEGORIES list (with legacy ``persons``/``conclave``/
    ``cargo`` fallbacks that no longer exist on disk) is gone.

    Also supports path-style refs like "acme/MEMORY.md" by extracting the
    leading dir as the agent slug.
    """
    if not AGENTS_DIR.exists():
        return None

    categories = _load_categories_from_registry()
    if not categories:
        # Registry unavailable — fall back to disk scan so the hook stays
        # functional on first run / fresh clones.
        categories = _categories_from_disk()

    # Path-style ref: "acme/MEMORY.md" → slug = "acme"
    if "/" in agent_name:
        slug = agent_name.split("/", 1)[0]
    else:
        slug = agent_name

    # Try exact dir-name match first (case-insensitive)
    ...
    for category in categories:
        ...
        for agent_dir in category_path.iterdir():
            if agent_dir.is_dir() and agent_dir.name.lower() == slug_lower:
                return agent_dir

    # Fallback: substring match (legacy DNA-XX style → "xx" in dir name)
    normalized = slug.upper().replace(" ", "-")
    ...
    for category in categories:
        ...
        for agent_dir in category_path.iterdir():
            if agent_dir.is_dir() and normalized in agent_dir.name.upper():
                return agent_dir
            # Also walk one level deep for nested categories (cargo/sales/closer)
            if agent_dir.is_dir():
                for sub in agent_dir.iterdir():
                    if sub.is_dir() and normalized in sub.name.upper():
                        return sub
...
def process_batch(batch_path: str) -> dict:
    ...
    try:
        ...
        # C-17-CASCADE-DISPATCHER (2026-05-12): if destinations are in canonical
        # format (path-style filenames with extensions), route through the new
        # canonical dispatcher which resolves to actual file paths via
        # validator's multi-bucket resolvers. Legacy heavy dispatchers run only
        # if destination names are in the legacy format (CAPS-AGENT-NAMES, no
        # extension, no slash).
        def _is_canonical(dest_list):
            """Canonical = name contains "/" or ends in .md/.yaml/.json."""
            if not dest_list:
                return False
            first = dest_list[0].get("name", "")
            return "/" in first or first.endswith((".md", ".yaml", ".yml", ".json"))

        use_canonical = any(
            _is_canonical(destinations[s]) for s in ("agents", "dnas", "dossiers", "playbooks")
        )

        if use_canonical:
            # Canonical dispatch: append batch reference footer to each file
            for section in ("agents", "dnas", "dossiers", "playbooks"):
                section_actions = []
                for dest in destinations[section]:
                    action = cascade_file_canonical(
                        section=section,
                    ...
                    log_cascading_action(
                        {
                            "type": f"{section[:-1]}_cascade_canonical",
                            "batch_id": batch_id,
                            "destination": dest["name"],
                            "action": action,
                        }
                    )
                result[section] = section_actions
        else:
            # Legacy dispatch (kept for backward compat with old batches)
        ...
        try:
            ...
            # H-16 fix (2026-05-12): script lives in .claude/scripts/, not scripts/
