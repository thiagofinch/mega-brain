...
def update_pending_file(priority, item, action="add"):
    ...
    high_count = (
        len(re.findall(r"## Alta Prioridade.*?(?=##)", content, re.DOTALL)[0].split("- [ ]")) - 1
        if "## Alta Prioridade" in content
        else 0
    )
    medium_count = (
        len(re.findall(r"## Media Prioridade.*?(?=##)", content, re.DOTALL)[0].split("- [ ]")) - 1
        if "## Media Prioridade" in content
        else 0
    )
    low_count = (
        len(re.findall(r"## Baixa Prioridade.*?(?=##)", content, re.DOTALL)[0].split("- [ ]")) - 1
        if "## Baixa Prioridade" in content
        else 0
    )
    waiting_count = (
        len(re.findall(r"## Aguardando.*?(?=##)", content, re.DOTALL)[0].split("- ")) - 1
        if "## Aguardando" in content
        else 0
    )
    ...
    content = re.sub(
        r"\| Alta Prioridade \| \d+ \|", f"| Alta Prioridade | {high_count} |", content
    )
    content = re.sub(
        r"\| Media Prioridade \| \d+ \|", f"| Media Prioridade | {medium_count} |", content
    )
    content = re.sub(
        r"\| Baixa Prioridade \| \d+ \|", f"| Baixa Prioridade | {low_count} |", content
    )
    content = re.sub(
        r"\| Aguardando Usuario \| \d+ \|", f"| Aguardando Usuario | {waiting_count} |", content
    )
    content = re.sub(
        r"\| \*\*TOTAL PENDENTE\*\* \| \*\*\d+\*\* \|",
        f"| **TOTAL PENDENTE** | **{total}** |",
        content,
    )
...
def log_update(action, item):
    ...
    log_entry = {"timestamp": datetime.now().isoformat(), "action": action, "item": item[:200]}
...
def main():
    """Funcao principal do hook (side-effect only, no context injection)."""
    try:
        # Ler input do hook
        ...
        # Side-effect only — silent, no stdout

    except Exception:
        # Fail-open — nunca bloquear por erro no hook
        pass
