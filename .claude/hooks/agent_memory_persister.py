#!/usr/bin/env python3
# STUB — W1-001.6 (2026-04-16): Logic consolidated into engine/memory_manager.py
# Archive: .claude/hooks/_archived/agent_memory_persister.py
"""
Agent Memory Persister Hook — stub delegate to engine/memory_manager.py

Persists session learnings to the active agent's memory file at session end.
LIFECYCLE:
  READ:  memory_hints_injector.py (UserPromptSubmit) -> loads canonical MEMORY.md via ecosystem-registry.yaml
...
- Resolves canonical MEMORY.md path via ecosystem-registry.yaml (resolve_agent_path.py)
- Appends a session entry to the resolved path
- Fallback to .data/agent_memory/{slug}/MEMORY.md for unindexed agents
...
# Import canonical path resolver (MOD-001: writes to agent directories, not .data/agent_memory/)
try:
