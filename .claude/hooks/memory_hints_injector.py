...
INTEGRATION:
  ...
  - Reads from canonical agent path via ecosystem-registry.yaml (resolve_agent_path.py)
  - Fallback to .data/agent_memory/ for unindexed agents
...
# Import canonical path resolver (MOD-001: reads from agent directories, not .data/agent_memory/)
try:
...
try:
    ...
    from engine.intelligence.context_scorer import score_context_budget as _scb
...
def read_agent_memory_hints(agent_slug, max_lines, max_tokens):
    ...
    Uses canonical path resolver (ecosystem-registry.yaml) with .data/agent_memory/ fallback.
    """
    if _HAS_RESOLVER:
