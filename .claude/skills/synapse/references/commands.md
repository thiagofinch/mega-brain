...
1. **Mode directives** — Switch response behavior (`*brief`, `*dev`, etc.)
2. **`*synapse` sub-commands** — Query and manage engine state
3. **CRUD operations** — Create, modify, and manage domains and rules

## Mode Directives (L7 Slash-Skill Layer)

These commands switch the response mode for the current session. They are detected by L7 (slash-skill processor) and inject mode-specific rules.
...
**Note:** These are read-only operations handled by the L7 slash-skill processor in the hook. They do not modify any files.

## CRUD Operations

These commands modify domain files and the manifest. They are implemented as Claude Code skills in `.claude/skills/synapse/`.

### Router

All CRUD operations go through the skill: `.claude/skills/synapse/SKILL.md`
...
| `*synapse command` | `tasks/create-command.md` | Create a new L7 routing definition |
...
**Create a new L7 routing definition:**
```
*synapse command
```
...
Read-state directives   -> HOOK L7 (*synapse status, *synapse debug, *brief, *dev)
Write-file CRUD ops     -> COMMAND (*synapse create, *synapse add, *synapse toggle)
```
...
| `.claude/skills/synapse/references/commands.md` | CRUD command reference |
| `.claude/skills/synapse/assets/` | Domain and manifest templates |
| `.claude/skills/synapse/references/manifest.md` | Parser format reference |
