...
**MODO EXPANDIDO (quando tribunal_findings presente no contexto):**
Quando o deliberation gate (DELIBERATION-GATE-1.0) disparou o tribunal e `tribunal_findings` está disponível:
- Tratar `tribunal_findings` como fonte adicional — mesmo mecanismo de incorporação/justificativa usado para advogado-diabo attacks
- Recalibrar confidence com ajuste de tribunal (ver fórmula em sintetizador-conclave.md)
- Reportar no [CONFIDENCE] breakdown: `- Tribunal findings (gate triggered): {+/-N}%`
- Reportar round1_confidence e final_confidence no Decision Status
- NUNCA receber argumentos integrais de sub-agentes do tribunal — apenas o tribunal_summary de uma linha (AC6, sycophancy mitigation)
