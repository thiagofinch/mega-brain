...
### 5. CONSOLIDACAO: Gerar Arquivos Standalone L9 e L10 (OBRIGATORIO)

APOS extrair obsessions e paradoxes para o INSIGHTS-STATE.json (Secoes 2 e 3 acima),
voce DEVE TAMBEM gerar arquivos YAML standalone por pessoa. Estes arquivos sao
a fonte canonica para as camadas L9 (Obsessions) e L10 (Paradoxes) do DNA.

O INSIGHTS-STATE.json continua sendo atualizado normalmente (backward compat).
Os arquivos standalone sao OUTPUT ADICIONAL.

#### 5.1 Gerar OBSESSIONS.yaml (L9)

Para cada pessoa processada, gerar o arquivo:

```
knowledge/external/dna/persons/{slug}/OBSESSIONS.yaml
```

Seguir EXATAMENTE o schema definido no template:
`core/templates/agents/OBSESSIONS-TEMPLATE.yaml`

Campos obrigatorios no cabecalho:
- `pessoa`: Nome canonico da pessoa
- `versao`: "1.0.0" (ou incrementar se ja existir)
- `data_extracao`: Data atual em ISO-8601
- `layer`: "L9"
- `layer_name`: "Obsessions"
- `total_obsessions`: Contagem total de obsessoes extraidas
- `source_pipeline_step`: "Step 7 - Identity Extraction"

Cada obsessao na lista `obsessions:` deve conter TODOS os campos obrigatorios
definidos na Secao 2 deste prompt (id, nome, intensidade, status, descricao,
manifestacoes, chunk_ids, insight_ids, quotes, confianca).

REGRA: Os dados devem ser IDENTICOS aos escritos no INSIGHTS-STATE.json.
O arquivo standalone e uma PROJECAO estruturada, nao uma re-extracao.

#### 5.2 Gerar PARADOXES.yaml (L10)
...
Seguir EXATAMENTE o schema definido no template:
`core/templates/agents/PARADOXES-TEMPLATE.yaml`
...
- `total_paradoxes`: Contagem total de paradoxos extraidos
- `source_pipeline_step`: "Step 7 - Identity Extraction"

Cada paradoxo na lista `paradoxes:` deve conter TODOS os campos obrigatorios
definidos na Secao 3 deste prompt (id, nome, lado_a, lado_b, descricao_tensao,
resolucao, chunk_ids_lado_a, chunk_ids_lado_b, insight_ids, quotes, tipo,
produtivo, confianca).

REGRA: Os dados devem ser IDENTICOS aos escritos no INSIGHTS-STATE.json.

#### 5.3 Regras de Consolidacao

- Se o diretorio `knowledge/external/dna/persons/{slug}/` nao existe, CRIAR
- Se OBSESSIONS.yaml ou PARADOXES.yaml ja existem, MESCLAR (mesma logica de incrementalidade da Secao 4)
- O `{slug}` e o slug da pessoa em lowercase-kebab-case (ex: `alex-hormozi`)
- Gerar AMBOS os arquivos, mesmo que paradoxes esteja vazio (gerar com lista vazia)
...
# STANDALONE FILES (L9/L10)
[ ] CP-POST-MCE2.O: OBSESSIONS.yaml existe em knowledge/external/dna/persons/{slug}/ para cada pessoa processada
[ ] CP-POST-MCE2.P: PARADOXES.yaml existe em knowledge/external/dna/persons/{slug}/ para cada pessoa processada
[ ] CP-POST-MCE2.Q: OBSESSIONS.yaml e YAML valido (parseable sem erros) com campo layer: "L9"
[ ] CP-POST-MCE2.R: PARADOXES.yaml e YAML valido (parseable sem erros) com campo layer: "L10"
[ ] CP-POST-MCE2.S: Dados em OBSESSIONS.yaml sao consistentes com obsessions no INSIGHTS-STATE.json (mesmos IDs e contagem)
[ ] CP-POST-MCE2.T: Dados em PARADOXES.yaml sao consistentes com paradoxes no INSIGHTS-STATE.json (mesmos IDs e contagem)
...
Se CP-POST-MCE2.O falhar: ⛔ EXIT("OBSESSIONS.yaml nao foi gerado -- executar Secao 5.1")
Se CP-POST-MCE2.P falhar: ⛔ EXIT("PARADOXES.yaml nao foi gerado -- executar Secao 5.2")
Se CP-POST-MCE2.Q falhar: ⛔ EXIT("OBSESSIONS.yaml invalido ou sem campo layer: L9")
Se CP-POST-MCE2.R falhar: ⛔ EXIT("PARADOXES.yaml invalido ou sem campo layer: L10")
Se CP-POST-MCE2.S falhar: ⚠️ WARN("OBSESSIONS.yaml e INSIGHTS-STATE.json divergem -- re-consolidar")
Se CP-POST-MCE2.T falhar: ⚠️ WARN("PARADOXES.yaml e INSIGHTS-STATE.json divergem -- re-consolidar")
```
