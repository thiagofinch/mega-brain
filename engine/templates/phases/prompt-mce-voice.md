...
> **Input A:** `/artifacts/insights/INSIGHTS-STATE.json` (com campos MCE-1 e MCE-2)
> **Input B:** `/artifacts/chunks/CHUNKS-STATE.json` (texto original dos chunks)
> **Input C:** `knowledge/external/dna/persons/{slug}/OBSESSIONS.yaml` (L9 standalone, com fallback para INSIGHTS-STATE.json)
> **Input D:** `knowledge/external/dna/persons/{slug}/PARADOXES.yaml` (L10 standalone, com fallback para INSIGHTS-STATE.json)
...
[ ] CP-MCE3.G: OBSESSIONS.yaml existe em knowledge/external/dna/persons/{slug}/ (L9 standalone)
[ ] CP-MCE3.H: PARADOXES.yaml existe em knowledge/external/dna/persons/{slug}/ (L10 standalone)
...
Se CP-MCE3.G falhar: ⚠️ WARN - "OBSESSIONS.yaml standalone nao encontrado. Fallback: usando campo obsessions do INSIGHTS-STATE.json"
Se CP-MCE3.H falhar: ⚠️ WARN - "PARADOXES.yaml standalone nao encontrado. Fallback: usando campo paradoxes do INSIGHTS-STATE.json"
```
...
- OBSESSIONS.yaml (L9): obsessoes standalone da pessoa (fonte canonica)
- PARADOXES.yaml (L10): paradoxos standalone da pessoa (fonte canonica)
- value_hierarchy: para calibrar tone_profile
...
Contem campos: persons, themes, behavioral_patterns, value_hierarchy.
Usar principalmente:
...
### Input C: OBSESSIONS.yaml (L9 - Standalone)

Arquivo: `knowledge/external/dna/persons/{slug}/OBSESSIONS.yaml`

Contem as obsessoes extraidas na Etapa MCE-2 como arquivo standalone.
Usar para calibrar tone_profile (obsessoes influenciam intensidade e foco vocal).

**FALLBACK:** Se o arquivo OBSESSIONS.yaml NAO existir para uma pessoa,
usar o campo `obsessions` do INSIGHTS-STATE.json como fonte alternativa.

### Input D: PARADOXES.yaml (L10 - Standalone)

Arquivo: `knowledge/external/dna/persons/{slug}/PARADOXES.yaml`

Contem os paradoxos extraidos na Etapa MCE-2 como arquivo standalone.
Usar para detectar tensoes vocais (paradoxos geram padroes de comunicacao contrastantes).

**FALLBACK:** Se o arquivo PARADOXES.yaml NAO existir para uma pessoa,
usar o campo `paradoxes` do INSIGHTS-STATE.json como fonte alternativa.
