...
TRIGGER_CONFIG_PATH = (
    BASE_DIR.parent / "patterns" / "trigger_config.yaml"
)  # H-09b fix 2026-05-10 — path canonico em engine/patterns/, nao engine/intelligence/scripts/
...
def get_thresholds():
    ...
    Priority order (Story MCE-11.19):
      1. thresholds.yaml (central source-of-truth)
      2. trigger_config.yaml (legacy config -- still respected as override)
      3. Hardcoded defaults (backward compat)
    """
    # Try central thresholds.yaml first (MCE-11.19)
    try:
        from engine.intelligence.pipeline.config.threshold_loader import get_threshold as _get_th

        yaml_fuzzy = _get_th("entity_resolution.fuzzy_threshold")
        yaml_merge = _get_th("entity_resolution.auto_merge_threshold")
    except Exception:
        yaml_fuzzy = None
        yaml_merge = None

    # Legacy override via trigger_config.yaml (still respected if present)
    config = load_trigger_config()
    ec = config.get("thresholds", {}).get("entity_canonicalization", {})

    return {
        "fuzzy_threshold": ec.get(
            "fuzzy_threshold", yaml_fuzzy if yaml_fuzzy is not None else DEFAULT_FUZZY_THRESHOLD
        ),
        "auto_merge_threshold": ec.get(
            "auto_merge_threshold",
            yaml_merge if yaml_merge is not None else DEFAULT_AUTO_MERGE_THRESHOLD,
        ),
