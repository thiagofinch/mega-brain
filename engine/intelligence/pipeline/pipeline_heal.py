...
class PipelineHealDetector:
    ...
    def _check_agent_memory(self, sid: str, slug: str) -> CheckResult:
        """P7.4: Verify agent memory file has this source_id.

        Checks lowercase (canonical) first, then UPPERCASE as backward-compat
        fallback for cargo agents using the legacy naming convention.
        """
        # Canonical lowercase first (external/business/personal agents)
        mem_path = f"agents/external/{slug}/memory.md"
        full = self.root / mem_path
        if not full.exists():
            # Fallback: UPPERCASE for cargo agents and legacy files
            mem_path = f"agents/external/{slug}/MEMORY.md"
            full = self.root / mem_path
        if not full.exists():
            return CheckResult(
                ...
                "Agent MEMORY",
                False,
                f"No memory.md or MEMORY.md at agents/external/{slug}/",
                f"Create agents/external/{slug}/memory.md with {sid} insights",
            )
        ...
        return CheckResult(
            ...
            "Agent MEMORY",
            False,
            f"memory file exists but no {sid} ref",
