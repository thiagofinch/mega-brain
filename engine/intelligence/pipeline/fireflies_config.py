...
def load_config() -> FirefliesConfig:
    """
    Load FirefliesConfig from the canonical config module.
    ...
    raw_domains = get_config("COMPANY_DOMAINS") or get_config("READ_AI_COMPANY_DOMAINS", "")
    ...
    raw_keywords = get_config("COMPANY_KEYWORDS") or get_config("READ_AI_COMPANY_KEYWORDS", "")
    keywords = _parse_csv(raw_keywords) or _DEFAULT_COMPANY_KEYWORDS

    # Company name: try COMPANY_NAME, then READ_AI_COMPANY_NAME
    company_name = get_config("COMPANY_NAME") or get_config("READ_AI_COMPANY_NAME", "")

    cfg = FirefliesConfig(
        api_key=get_config("FIREFLIES_API_KEY", ""),
        graphql_url=get_config(
        ...
        tag_prefix=get_config("FIREFLIES_TAG_PREFIX", "MEET"),
        poll_interval_minutes=int(get_config("FIREFLIES_POLL_INTERVAL", "5")),
        page_size=int(get_config("FIREFLIES_PAGE_SIZE", "50")),
    )
    return cfg
