...
# ---------------------------------------------------------------------------
# SIGNAL WEIGHT LOADING (from YAML, with hardcoded fallback)
# ---------------------------------------------------------------------------

# Default weights -- used as fallback if YAML is missing or malformed.
# These MUST stay in sync with scripts/scope_classifier_weights.yaml.
_DEFAULT_WEIGHTS: dict = {
    "path": {"inbox_weight": 10, "workspace_weight": 8},
    "participant": {
        "organizer": 5,
        "attendee": 2,
        "owner": 3,
    ...
    "entity": {"expert": 5, "team": 3, "owner": 2},
    "content_type": {"course": 8, "meeting": 6, "personal": 8, "template": 4},
    "cognitive": {"first_person": 3, "teaching": 4, "discussion": 3},
    "topic": {"company_keyword": 3, "expert_markers": 3, "business_markers": 3},
    "thresholds": {"cascade_threshold": 5, "text_sample_size": 5000},
}

_WEIGHTS_YAML_PATH = (
    Path(__file__).resolve().parents[3] / "scripts" / "scope_classifier_weights.yaml"
)


def _load_weights() -> dict:
    """Load classification weights from YAML file.

    Falls back to ``_DEFAULT_WEIGHTS`` with a warning if the YAML file
    is missing or cannot be parsed. Individual missing keys are also
    filled from defaults so partial YAML files degrade gracefully.
    ...
    if not _WEIGHTS_YAML_PATH.exists():
        logger.warning(
            "Weights YAML not found at %s; using hardcoded defaults",
            _WEIGHTS_YAML_PATH,
        )
        return _DEFAULT_WEIGHTS.copy()

    try:
        with open(_WEIGHTS_YAML_PATH, encoding="utf-8") as f:
            loaded = yaml.safe_load(f)

        if not isinstance(loaded, dict):
            logger.warning("Weights YAML is not a dict; using hardcoded defaults")
            return _DEFAULT_WEIGHTS.copy()

        # Deep-merge: use loaded values, fill gaps from defaults
        merged: dict = {}
        for section, defaults in _DEFAULT_WEIGHTS.items():
            if section not in loaded:
                merged[section] = defaults
                continue
            if isinstance(defaults, dict) and isinstance(loaded[section], dict):
        ...
        logger.warning(
            "Failed to parse weights YAML at %s; using hardcoded defaults",
            _WEIGHTS_YAML_PATH,
            exc_info=True,
        )
        return _DEFAULT_WEIGHTS.copy()
...
# ---------------------------------------------------------------------------
# SIGNAL WEIGHT CONSTANTS (resolved from YAML or defaults)
# ---------------------------------------------------------------------------

# Signal 1: Path Analysis
PATH_INBOX_WEIGHT = _W["path"]["inbox_weight"]
PATH_WORKSPACE_WEIGHT = _W["path"]["workspace_weight"]

# Signal 2: Participant Analysis
PARTICIPANT_ORGANIZER = _W["participant"]["organizer"]
PARTICIPANT_ATTENDEE = _W["participant"]["attendee"]
PARTICIPANT_OWNER = _W["participant"]["owner"]
PARTICIPANT_EXTERNAL = _W["participant"]["external"]
PARTICIPANT_ALL_INTERNAL = _W["participant"]["all_internal"]
PARTICIPANT_PERSONAL_MEETING = _W["participant"]["personal_meeting"]

# Signal 3: Entity Detection
ENTITY_EXPERT = _W["entity"]["expert"]
ENTITY_TEAM = _W["entity"]["team"]
ENTITY_OWNER = _W["entity"]["owner"]

# Signal 4: Content Type Detection
CONTENT_COURSE = _W["content_type"]["course"]
CONTENT_MEETING = _W["content_type"]["meeting"]
CONTENT_PERSONAL = _W["content_type"]["personal"]
CONTENT_TEMPLATE = _W["content_type"]["template"]

# Signal 5: Cognitive Patterns
COGNITIVE_FIRST_PERSON = _W["cognitive"]["first_person"]
COGNITIVE_TEACHING = _W["cognitive"]["teaching"]
COGNITIVE_DISCUSSION = _W["cognitive"]["discussion"]

# Signal 6: Topic Keywords
TOPIC_COMPANY_KEYWORD = _W["topic"]["company_keyword"]
TOPIC_EXPERT_MARKERS = _W["topic"]["expert_markers"]
TOPIC_BUSINESS_MARKERS = _W["topic"]["business_markers"]

# Thresholds
CASCADE_THRESHOLD = _W["thresholds"]["cascade_threshold"]
TEXT_SAMPLE_SIZE = _W["thresholds"]["text_sample_size"]
...
def _load_known_experts() -> set[str]:
    ...
    try:
        from engine.intelligence.pipeline.inbox_organizer import ENTITY_ALIASES
...
def _load_owner_aliases() -> list[str]:
    ...
    try:
        from engine.intelligence.pipeline.pipeline_router import get_owner_aliases
...
def _load_owner_email() -> str:
    ...
    try:
        from engine.intelligence.pipeline.pipeline_router import OWNER_EMAIL
...
_TEMPLATE_PATTERNS = re.compile(
    r"(?:\bscript\b|\btemplate\b|roteiro|\bsop\b|" r"checklist|playbook|processo|procedure)",
    re.IGNORECASE,
)
...
def classify(ctx: ClassificationContext) -> ScopeDecision:
    ...
    Example::
        ...
        from engine.intelligence.pipeline.scope_classifier import classify, ClassificationContext
