...
are merged using **structured field diff** (not regex text merge).

Merge strategy (per-field):
    - Header metadata (date, version, total): last_write_wins
    - Sources list: append_unique (no duplicates)
    - Domain sections (## Heading): deep_merge (append new insights, keep existing)
...
Usage:
    ...
    python3 -m core.intelligence.pipeline.dossier_compiler --person "Cole Gordon" --dry-run

Version: 2.0.0
Date: 2026-04-16
Story: W3-001.2 (structured diff merge)
"""
...
# ---------------------------------------------------------------------------
# STRUCTURED DOSSIER MODEL (for merge)
# ---------------------------------------------------------------------------
# Merge strategies per field type:
#   - last_write_wins: scalar fields (date, version, total) -- newer value wins
#   - append_unique:   list fields (sources) -- union by identity, no duplicates
#   - deep_merge:      domain sections -- keep existing insights, append new ones


@dataclass
class ParsedDossier:
    """Structured representation of a dossier markdown file.

    Instead of treating the dossier as free text, we parse it into typed fields.
    This allows dict-level merge with explicit strategy per field, avoiding the
    fragility of regex-based text splicing.

    Merge strategies:
        - header_meta (date, version, total): last_write_wins
        - sources: append_unique (deduplicated list)
        - domain_sections: deep_merge (existing insights kept, new appended)
    """
    ...
    # domain_id -> list of raw insight text blocks (already formatted)
    domain_sections: dict[str, list[str]] = field(default_factory=dict)
    # domain_id -> human-readable label
    domain_labels: dict[str, str] = field(default_factory=dict)
    # Track which insight IDs are present (for dedup)
    insight_ids: set[str] = field(default_factory=set)
    # Domain ordering (preserves original order from file)
    domain_order: list[str] = field(default_factory=list)


def _parse_dossier(content: str) -> ParsedDossier:
    """Parse a dossier markdown string into a ParsedDossier struct.

    Extracts header metadata and splits domain sections into structured fields
    so merge can operate at the dict level, not the text level.
    """
    parsed = ParsedDossier()

    # -- Extract header metadata --
    name_match = re.search(r"^# DOSSIER -- (.+)$", content, re.MULTILINE)
    if name_match:
        parsed.name = name_match.group(1).strip()

    bucket_match = re.search(r"\*\*Tipo:\*\*\s*Pessoa\s*\(([^)]+)\)", content)
    if bucket_match:
        parsed.bucket_label = bucket_match.group(1).strip()

    sources_match = re.search(r"\*\*Sources:\*\*\s*(.+)$", content, re.MULTILINE)
    if sources_match:
        raw = sources_match.group(1).strip()
        parsed.sources = [s.strip() for s in raw.split(",") if s.strip()]

    date_match = re.search(r"\*\*Ultima atualizacao:\*\*\s*(\S+)", content)
    if date_match:
    ...
    total_match = re.search(r"\*\*Total insights:\*\*\s*(\d+)", content)
    if total_match:
        parsed.total_insights = int(total_match.group(1))

    # -- Extract insight IDs from citations --
    parsed.insight_ids = _extract_existing_ids(content)

    # -- Split domain sections --
    # Find all ## headings and their content (everything until next ## or EOF)
    section_splits = re.split(r"(?=^## )", content, flags=re.MULTILINE)

    for section_text in section_splits:
        heading_match = re.match(r"^## (.+)$", section_text, re.MULTILINE)
        if not heading_match:
            continue
        domain_label = heading_match.group(1).strip()

        # Extract individual insight blocks (lines starting with "- **")
        insight_blocks: list[str] = []
        current_block_lines: list[str] = []

        body = section_text[heading_match.end() :]
        for line in body.split("\n"):
            stripped = line.strip()
            if stripped.startswith("- **"):
                # Start of new insight block
                if current_block_lines:
                    insight_blocks.append("\n".join(current_block_lines))
                current_block_lines = [line]
            elif stripped == "---":
                # Section separator, stop
                if current_block_lines:
                    insight_blocks.append("\n".join(current_block_lines))
                    current_block_lines = []
                break
            elif current_block_lines:
                # Continuation of current insight block (quote, citation, etc.)
                current_block_lines.append(line)

        if current_block_lines:
            insight_blocks.append("\n".join(current_block_lines))

        # Use domain_label as key (we don't reverse-map to domain_id here)
        if insight_blocks:
            parsed.domain_sections[domain_label] = insight_blocks
            parsed.domain_labels[domain_label] = domain_label
            if domain_label not in parsed.domain_order:
                parsed.domain_order.append(domain_label)

    return parsed


def _merge_dossiers(
    existing: ParsedDossier,
    new_domain_insights: dict[str, list[Insight]],
    taxonomy: DomainTaxonomy,
    person: PersonInsights,
) -> str:
    """Merge new insights into an existing parsed dossier using structured field diff.

    Strategy per field:
        - date:             last_write_wins (update to now)
        - version:          last_write_wins (keep existing)
        - total_insights:   last_write_wins (recalculate)
        - sources:          append_unique (union of old + new, deduplicated)
        - domain_sections:  deep_merge (keep existing insight blocks,
                            append new ones; create new sections if needed)
    """
    now = dt.datetime.now(tz=dt.UTC).strftime("%Y-%m-%d")

    # -- last_write_wins: header metadata --
    merged_date = now
    merged_version = existing.version or "1.0.0"

    # -- append_unique: sources --
    ...
    # -- deep_merge: domain sections --
    # Start with a copy of existing sections
    merged_sections: dict[str, list[str]] = {}
    merged_labels: dict[str, str] = {}
    merged_order: list[str] = list(existing.domain_order)

    for label, blocks in existing.domain_sections.items():
        merged_sections[label] = list(blocks)
        merged_labels[label] = label

    # Append new insights per domain (only those not already present by ID)
    new_count = 0
    for domain_id, insights in new_domain_insights.items():
        domain_label = taxonomy.get_domain_label(domain_id)

        new_blocks: list[str] = []
        for ins in insights:
            if ins.id and ins.id in existing.insight_ids:
                continue  # Already in dossier, skip (dedup)
            new_blocks.append(_format_insight_block(ins))
            new_count += 1

        if not new_blocks:
            continue

        if domain_label in merged_sections:
            # deep_merge: append to existing section
            merged_sections[domain_label].extend(new_blocks)
        else:
            # New section that didn't exist before
            merged_sections[domain_label] = new_blocks
            merged_labels[domain_label] = domain_label
            if domain_label not in merged_order:
                merged_order.append(domain_label)

    # -- Recalculate total --
    total_insight_blocks = sum(len(blocks) for blocks in merged_sections.values())
    merged_total = max(existing.total_insights, total_insight_blocks)

    # -- Render back to markdown --
    header = DOSSIER_HEADER_TEMPLATE.format(
        name=existing.name,
        bucket_label=existing.bucket_label,
        sources=", ".join(merged_sources) if merged_sources else "INSIGHTS-STATE.json",
        date=merged_date,
    ...
    sections: list[str] = []
    for label in merged_order:
        blocks = merged_sections.get(label, [])
        if not blocks:
            continue
        insights_block = "\n\n".join(blocks)
        section = DOMAIN_SECTION_TEMPLATE.format(
...
def _merge_into_existing(
    ...
    """Merge new insights into an existing dossier using structured field diff.

    Replaces the old regex-based text splicing with a 3-step approach:
    1. Parse existing dossier markdown into a ParsedDossier struct
    2. Merge at the dict/field level with per-field strategies
    3. Render the merged struct back to markdown

    This is functionally equivalent to the old regex merge but far more robust
    because it operates on structured data, not raw text patterns.
    """
    parsed = _parse_dossier(existing)
    return _merge_dossiers(parsed, new_domain_insights, taxonomy, person)
