...
from engine.intelligence.pipeline.read_ai_config import ReadAIConfig, load_config
from engine.intelligence.pipeline.read_ai_gardener import PessoalGardener
from engine.intelligence.pipeline.read_ai_router import MeetingRouter
...
class ReadAIClient:
    ...
    def list_meetings(self, page_token: str | None = None) -> dict[str, Any]:
    ...
    def _load_access_token(self) -> str | None:
        ...
        from engine.intelligence.pipeline.read_ai_oauth import get_access_token

        return get_access_token()

    def _refresh_token(self) -> None:
        """Attempt to refresh expired token."""
        from engine.intelligence.pipeline.read_ai_oauth import cmd_refresh
...
class ReadAIHarvester:
    ...
    def run(self) -> dict[str, Any]:
        ...
        try:
            ...
            for i, meeting_summary in enumerate(all_meetings):
                ...
                # Gardener trigger
                if self.state.pessoal_since_garden >= self.config.gardener_trigger:
    ...
    def fetch_single(self, meeting_id: str) -> dict[str, Any]:
        ...
        if meeting_id in self.state.processed_ids:
            self._log(
                {"event": "single_fetch_skipped", "id": meeting_id, "reason": "already processed"}
            )
            # Find existing file
