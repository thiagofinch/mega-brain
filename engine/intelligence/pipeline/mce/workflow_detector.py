...
Usage::
    ...
    from engine.intelligence.pipeline.mce.workflow_detector import detect
...
try:
    from engine.paths import AGENTS_EXTERNAL, KNOWLEDGE_EXTERNAL, ROUTING
except ImportError:
...
try:
    from engine.intelligence.utils.agent_files import find_agent_file as _find_agent_file
except ImportError:
    _find_agent_file = None  # type: ignore[assignment]
...
def _check_agent(slug: str) -> bool:
    """Check if an agent directory with agent.md (or AGENT.md) exists.

    MCE-13.6: uses find_agent_file for case-insensitive lookup.
    """
    agent_dir = AGENTS_EXTERNAL / slug
    if _find_agent_file is not None:
        return _find_agent_file(agent_dir, "agent.md") is not None
    # Fallback for standalone (no engine.paths available)
    return (agent_dir / "AGENT.md").exists() or (agent_dir / "agent.md").exists()
