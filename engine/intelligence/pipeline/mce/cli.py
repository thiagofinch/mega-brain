...
Usage::
    ...
    # Intentionally pause a running session
    python -m core.intelligence.pipeline.mce.cli --persona alex-hormozi --pause

    # Pause at a specific step cursor
    python -m core.intelligence.pipeline.mce.cli --persona alex-hormozi --pause --step-cursor 5

    # Dry run (preview, no writes)
    python -m core.intelligence.pipeline.mce.cli --persona alex-hormozi --dry-run

    # Upgrade existing 5-layer cognitive DNA agent to full MCE (L1-L10)
...
from engine.intelligence.pipeline.mce.hook_bus import HookEvent, get_hook_bus
...
def cmd_init(
    ...
    *,
    ...
    Args:
        ...
        upgrade: If True, treats as brownfield upgrade from legacy 5-layer cognitive DNA.
...
def cmd_pause(slug: str, step_cursor: int | None = None, verbose: bool = False) -> int:
    """Intentionally pause a running pipeline session.

    Saves the current state, marks the machine as ``paused``, and emits
    the ``on_session_paused`` hook so observers can react (flush caches,
    save partial work, etc.).

    This is fundamentally different from crash recovery: the operator is
    choosing to stop, and ``cmd_resume()`` will return to this exact point.

    Args:
        slug: Persona slug.
        step_cursor: Optional step index within the current phase.
        verbose: Extra output.

    Returns:
        0 on success, 1 if pipeline cannot be paused.
    """
    _header(f"MCE Pipeline Pause: {slug}")

    sm = PipelineStateMachine(slug)

    if sm.state == "init":
        print("  No active pipeline to pause. Use --persona to start first.")
        print()
        return 1
    ...
    if sm.paused:
        ...
        _info("Paused from:", sm.paused_from or "(unknown)")
        _info("Step cursor:", str(sm.step_cursor) if sm.step_cursor is not None else "(none)")
        print()
        return 1

    # Capture pre-pause state for display.
    pre_pause_state = sm.state

    # Pause the session (saves state + records paused_from + step_cursor).
    sm.pause_session(step_cursor=step_cursor)

    _info("Paused from:", pre_pause_state)
    if step_cursor is not None:
        _info("Step cursor:", str(step_cursor))
    _info("State file:", str(sm.state_path))

    # Emit the on_session_paused hook.
    bus = get_hook_bus()
    bus.emit(
        HookEvent.ON_SESSION_PAUSED,
        {
            "slug": slug,
            "paused_from": pre_pause_state,
            "step_cursor": step_cursor,
        },
    )

    print()
    print("  Session paused. To resume later:")
    print(f"  python -m core.intelligence.pipeline.mce.cli --persona {slug} --resume")
    print()
    return 0
...
def cmd_resume(slug: str, verbose: bool = False) -> int:
    ...
    Detects whether the pipeline was intentionally paused (via ``--pause``)
    or is simply in a resumable mid-pipeline state.  When paused, uses
    ``resume_from_pause()`` to go back to the exact pre-pause state.
    When not paused, shows status information for the Claude Code MCE
    skill to pick up.

    This is NOT crash recovery.  Crash recovery uses ``recover_to_last()``.
    ...
    # Intentional pause path: use resume_from_pause() (not recover_to_last).
    if sm.paused and sm.paused_from is not None:
        paused_from = sm.paused_from
        step_cursor = sm.step_cursor

        _info("Was paused from:", paused_from)
        if step_cursor is not None:
            _info("Step cursor:", str(step_cursor))

        resumed_to = sm.resume_from_pause()
        _info("Resumed to:", resumed_to)

        # Emit the on_session_resumed hook.
        bus = get_hook_bus()
        bus.emit(
        ...
        print()
        print("  Session resumed from intentional pause.")
        print("  Open Claude Code and the MCE skill will continue from this state.")
        print()
        return 0

    # Generic resume path: pipeline is mid-execution (not paused).
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Intentionally pause the running pipeline (saves state for later resume)",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        default=False,
        help="Resume from last saved state (detects intentional pause vs mid-pipeline)",
    )
    parser.add_argument(
        "--step-cursor",
        type=int,
        default=None,
        help="Step index within current phase (used with --pause)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Upgrade existing 5-layer cognitive DNA agent to full MCE (L1-L10, forces brownfield mode)",
    )
    parser.add_argument(
        "--skip-checkpoint",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.pause:
        return cmd_pause(slug, step_cursor=args.step_cursor, verbose=args.verbose)
