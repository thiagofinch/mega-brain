...
class MetadataManager:
    ...
    def __init__(
        ...
        *,
        ...
        # MCE-2.1 AC1: ingest-time blocks consumed by log_generator (Steps 1, 2).
        # These fields are produced by cmd_ingest and persisted so that
        # log_generator._check_classification / _check_organization can detect
        # successful Step 1 / Step 2 from metadata.yaml alone.
    ...
    # -- FSM history integration --------------------------------------------

    def sync_from_fsm_history(self, history: list[dict[str, Any]]) -> int:
        """Derive completed phases from FSM transition history.

        Walks the history and marks phases as complete based on the
        ``to`` states that correspond to known phases.  Returns the
        count of newly marked phases.

        Args:
            history: Transition history from ``PipelineStateMachine.history``.
        """
        # Map FSM states to metadata phase names
        _state_to_phase: dict[str, str] = {
            "chunking": "chunking",
            "entity_resolution": "entity_resolution",
            "insight_extraction": "knowledge_extraction",
            "mce_behavioral": "mce_extraction",
        ...
        for entry in history:
            target = entry.get("to", "")
            phase = _state_to_phase.get(target)
            if phase and not self.is_phase_complete(phase):
                self.mark_phase_complete(phase, derived_from="fsm_history")
                newly_marked += 1
        return newly_marked
    ...
    # -- ingest-time setters (MCE-2.1 AC1) ----------------------------------

    def set_classification(self, payload: dict[str, Any]) -> None:
        """Persist the Step 1 (classification) block in metadata.yaml.

        Expected shape (produced by cmd_ingest):
        {primary_bucket, cascade_buckets, confidence, source_type, reasons}
        """
        if isinstance(payload, dict):
            self.classification = dict(payload)
            self.updated_at = _now_iso()

    def set_organization(self, payload: dict[str, Any]) -> None:
        """Persist the Step 2 (organization) block in metadata.yaml.

        Expected shape: {organized: int, ...}
        """
        if isinstance(payload, dict):
            self.organization = dict(payload)
            self.updated_at = _now_iso()

    def set_routing(self, payload: dict[str, Any]) -> None:
        """Persist the routing block (file movement record) in metadata.yaml.

        Expected shape: {action, moved_to, references_created}
        """
        if isinstance(payload, dict):
            self.routing = dict(payload)
            self.updated_at = _now_iso()

    def set_source_path(self, source_path: str) -> None:
        """Persist the original source file path (used by Step 2 fallback)."""
        if source_path:
    ...
    def save(self) -> Path:
        ...
        # MCE-2.1 AC1: persist ingest-time blocks only when populated.
        # Keeps backwards compatibility with metadata.yaml files written
        # before AC1 (those omit these keys entirely).
    ...
    def load(cls, slug: str) -> MetadataManager | None:
        ...
        # MCE-2.1 AC1: rehydrate ingest-time blocks (absent in legacy files).
        mgr.classification = data.get("classification", {}) or {}
        mgr.organization = data.get("organization", {}) or {}
        mgr.routing = data.get("routing", {}) or {}
        mgr.source_path = data.get("source_path", "") or ""
        mgr._path = path
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        # MCE-2.1 AC1: include ingest-time blocks in dict view when set.
        if self.classification:
