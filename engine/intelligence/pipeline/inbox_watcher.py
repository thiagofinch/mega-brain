...
# Directories to monitor -- sourced from engine.paths, never hardcoded.
...
# State and log paths from engine.paths ROUTING.
...
def process_file(
    ...
    *,
    ...
    if path.is_symlink():
        logger.warning("Skipping symlink (security guard): %s", path)
...
def startup_sweep(state: WatcherState, *, dry_run: bool = False) -> int:
    ...
    """
    from engine.intelligence.pipeline.inbox_organizer import (
        SKIP_NAMES,
    ...
    for bucket_key, inbox_path in WATCHED_DIRS.items():
        ...
        from engine.intelligence.pipeline.inbox_organizer import BUCKET_INBOXES
        ...
        for filepath in all_files:
            # Anti-symlink guard: skip symlinks during sweep.
            if filepath.is_symlink():
                logger.warning("Skipping symlink in startup sweep: %s", filepath)
                continue
