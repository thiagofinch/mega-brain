...
Usage:
    from engine.intelligence.pipeline.ss_bridge import ingest_pdf, is_ss_available

    if is_ss_available():
        output_path = ingest_pdf(Path("document.pdf"), "alex-hormozi")
    else:
        # Use fallback extractors
        from engine.intelligence.pipeline.extractors import extract_pdf
...
from engine.paths import KNOWLEDGE_BUSINESS, KNOWLEDGE_EXTERNAL, KNOWLEDGE_PERSONAL, ROUTING
...
def ingest_docx(
    ...
    if use_fallback and _is_fallback_enabled():
        ...
        try:
            from engine.intelligence.pipeline.extractors import extract_docx
