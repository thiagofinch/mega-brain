...
def log_creation(
    person_name: str, slug: str, agent_dir: Path, bucket: str, insight_count: int
) -> None:
...
def check_and_create_agent(
    person_name: str, state: dict | None = None, dry_run: bool = False
) -> dict:
    ...
    if insight_count < MCE_INSIGHT_THRESHOLD:
        result["action"] = "skip"
        result["reason"] = (
            f"Below threshold: {insight_count} insights " f"(need {MCE_INSIGHT_THRESHOLD}+)"
        )
        return result
    ...
    # MCE-13.6: resolve bucket-aware filename for idempotency check
    fnames = get_agent_filenames(agent_dir)
    agent_file = agent_dir / fnames["agent"]

    # Idempotency check: agent already exists (supports old UPPERCASE via find_agent_file)
    if agent_dir.exists() and find_agent_file(agent_dir, "agent.md") is not None:
        result["action"] = "noop"
    ...
    # Create agent directory and agent.md (lowercase for non-cargo)
    ...
    print(
        f"[CREATED] {person_name}: {agent_dir.relative_to(ROOT)}/{fnames['agent']} "
        f"({insight_count} insights, bucket={bucket})"
    )
    return result


def scan_all_persons(state: dict | None = None, dry_run: bool = False) -> list[dict]:
...
def _print_result_summary(results: list[dict]) -> None:
    ...
    for r in results:
        ...
        print(
            f"  [{icon}] {r['person']:<25} insights={insight_str:<6} "
