...
try:
    ...
    print(
        "AVISO: Google API nao disponivel. Instale: pip install google-auth-oauthlib google-api-python-client"
    )
...
COURSE_TO_FOLDER: dict = {
    ...
    # Example: {"Alex Hormozi": "ALEX HORMOZI", "G4": "education-methodologAO"}
}
...
def get_all_inbox_files() -> list:
    ...
    for root, dirs, filenames in os.walk(INBOX_PATH):
        ...
        dirs[:] = [d for d in dirs if not any(d.startswith(ignore) for ignore in IGNORE_FOLDERS)]
    ...
    try:
        ...
        for tab_name in tab_names:
            ...
            try:
                ...
                if (
                    len(values) < 4
                ):  # Precisa de pelo menos 4 linhas (titulo, header, header2, dados)
                    continue
                ...
                for row_idx, row in enumerate(
                    values[header_row_idx + 1 :], start=header_row_idx + 2
                ):
                    ...
                    item = {
                        ...
                        "normalized_transcricao": normalize_name(transcricao)
                        if transcricao
                        else "",
...
def run_de_para():
    ...
    for item in planilha_items:
        ...
        if match["match_type"] != "NO_MATCH" and match["confidence"] >= 0.7:
            ...
            matched.append({"planilha": item, "inbox": match})
        else:
            missing.append(
                {"planilha": item, "best_match": match if match["confidence"] > 0.3 else None}
            )
...
""")
    ...
    for f in extra:
        report["extra"].append(
            {"source": f["source"], "filename": f["filename"], "rel_path": f["rel_path"]}
        )
...
Exemplos:
  ...
  python de_para_completo.py --source "Jeremy"  # Filtra por fonte
    ...
    parser.add_argument(
        "--missing-only", action="store_true", help="Exibe apenas arquivos faltantes"
    )
