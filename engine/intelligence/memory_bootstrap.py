...
SYSTEM_GROUP_ROLES: dict[str, str] = {
    ...
    # "dev-ops" removed 2026-03-31 — superseded by pipeline-ops
    "pipeline-ops": "Pipeline Ops Agent -- MCE pipeline processing and knowledge extraction",
}

# Mapping from system sub-group to dispatch prefix.
# Squad routing uses prefixed names (kops-atlas, etc.) while
# AGENT-INDEX.yaml uses bare names (atlas, etc.). We create directories
# for BOTH so the hook writes and bootstrap reads are aligned.
DISPATCH_PREFIX: dict[str, str] = {
    "pipeline-ops": "kops",
    # "dev-ops": "dops" removed 2026-03-31
}
...
def extract_all_agents(index: dict) -> list[dict]:
    ...
    # System agents (nested by sub-group: conclave, dev-ops, pipeline-ops)
...
def _infer_capabilities(category: str, group: str | None, agent_id: str) -> list[str]:
    ...
    if category == "external":
        ...
        if group == "pipeline-ops":
            caps = ["mce-pipeline", "knowledge-extraction", "dna-processing", "dossier-compilation"]
        # dev-ops group removed 2026-03-31 — superseded by pipeline-ops
        elif group == "conclave":
