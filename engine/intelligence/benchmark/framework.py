...
Usage:
    from engine.intelligence.benchmark.framework import Benchmark, BenchmarkSuite
...
class BenchmarkSuite:
    ...
    def save_report(self, report: BenchmarkReport | None = None) -> Path:
        ...
        self._results_dir.mkdir(parents=True, exist_ok=True)
        report_file = (
            self._results_dir / f"suite-{self.name}-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
        )
...
def compare_benchmarks(current: list[BenchmarkResult], baseline: list[BenchmarkResult]) -> dict:
