...
# Project root -- from engine.paths single source of truth
from engine.paths import ROOT as _PROJECT_ROOT
...
def graph_query(
    ...
    *,
    ...
    for ent in entity_list:
        searchable = " ".join(
            str(ent.get(field, "")) for field in ("name", "label", "id", "person")
        ).lower()
...
def quality_report(
    ...
    *,
    ...
    avg_score = (
        sum(s.get("total", s.get("score", 0)) for s in recent) / len(recent) if recent else 0.0
    )
