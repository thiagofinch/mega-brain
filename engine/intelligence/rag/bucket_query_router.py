...
def detect_bucket(question: str) -> Bucket:
    ...
    TRANSCEND T5: heuristic keyword scoring is the primary mechanism.
    S10 enhancement: TF-IDF+SVM classifier used as hint when heuristic
    returns "all" (ambiguous/zero-score case).
    ...
    # Check for ties or zero scores — ask TF-IDF+SVM classifier for hint (S10)
    if max_score == 0 or len([b for b, s in scores.items() if s == max_score]) > 1:
        try:
            from .query_classifier import HIGH_CONFIDENCE_THRESHOLD, classify_query

            ml_bucket, ml_confidence = classify_query(question)
            if ml_confidence >= HIGH_CONFIDENCE_THRESHOLD and ml_bucket in VALID_BUCKETS:
                return ml_bucket  # type: ignore[return-value]
        except Exception:
            pass
        return "all"
...
def detect_bucket_with_confidence(question: str) -> tuple[str, float, str]:
    """Detect bucket with confidence and source metadata for QueryTrace.

    Returns:
        (bucket, confidence, source) where source is "heuristic" or "classifier"
    """
    scores: dict[str, int] = {"external": 0, "business": 0, "personal": 0}
    ...
    if max_score > 0 and len(winners) == 1:
        # Heuristic has clear winner
        total = sum(scores.values()) or 1
        confidence = max_score / total
        return (winners[0], round(confidence, 3), "heuristic")

    # Ambiguous — try classifier
    try:
        from .query_classifier import classify_query

        ml_bucket, ml_confidence = classify_query(question)
        if ml_bucket in VALID_BUCKETS:
            return (ml_bucket, ml_confidence, "classifier")
    except Exception:
        pass

    return ("all", 0.0, "heuristic")
