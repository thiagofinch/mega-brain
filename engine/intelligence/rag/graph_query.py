...
# ---------------------------------------------------------------------------
# IRON LAW QUERY FUNCTIONS (W2-001.9)
# ---------------------------------------------------------------------------
def check_backlinks(node_id: str, graph: KnowledgeGraph | None = None) -> list[dict]:
    """Return all nodes that have an edge pointing TO ``node_id``.

    Each result contains the source node id, the relationship type,
    the edge weight, and whether the edge is a backlink (auto-generated)
    or a direct link.

    Args:
        node_id: The target node to check inbound links for.
        graph: Optional KnowledgeGraph; uses the singleton if omitted.

    Returns:
        List of dicts with keys: source, rel_type, weight, is_backlink.
    """
    g = graph or get_graph()
    inbound: list[dict] = []
            ...
            inbound.append(
                {
                    ...
                    "weight": edge.weight,
                    "is_backlink": edge.metadata.get("type") == "backlink",
                }
            )
...
def find_dead_links(graph: KnowledgeGraph | None = None) -> list[dict]:
    """Return edges whose target node does not exist in the graph.

    A dead link means the edge references an entity id that was never
    registered via ``add_entity``.

    Args:
        graph: Optional KnowledgeGraph; uses the singleton if omitted.

    Returns:
        List of dicts with keys: source, target, rel_type.
    """
    g = graph or get_graph()
    dead: list[dict] = []

    for edge in g.edges:
        if edge.target not in g.entities:
            dead.append(
                {
                    "source": edge.source,
                    "target": edge.target,
...
def find_orphans(graph: KnowledgeGraph | None = None) -> list[dict]:
    """Return nodes that have zero inbound AND zero outbound edges.

    An orphan is completely disconnected from the rest of the graph --
    no edge touches it in either direction.

    Args:
        graph: Optional KnowledgeGraph; uses the singleton if omitted.

    Returns:
        List of dicts with keys: id, type, label.
    """
    g = graph or get_graph()
