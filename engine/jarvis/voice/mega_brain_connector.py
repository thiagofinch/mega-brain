...
class MegaBrainConnector:
    ...
    def _load_state(self) -> dict:
        ...
        try:
            with open(self.config.JARVIS_STATE_PATH, encoding="utf-8") as f:
                return json.load(f)
        except FileNotFoundError:
            return self._create_default_state()
        except json.JSONDecodeError:
            print("[WARN] STATE.json corrompido, criando novo...")
            return self._create_default_state()
    ...
    def _create_default_state(self) -> dict:
        ...
        state = {
            ...
            "voice_system": {"version": "1.0.0", "last_session": None, "total_interactions": 0},
            "mission": {
                "id": None,
                "phase": 0,
                "subphase": 0,
            ...
            "agents": {"active": [], "last_consulted": None},
            "context": {"last_10_actions": [], "pending_questions": [], "session_start": None},
        }
...
"""
    ...
    def add_action(self, action: str):
        ...
        self.state["context"]["last_10_actions"].append(f"[{timestamp}] {action}")

        # Mantém apenas últimas 10
        self.state["context"]["last_10_actions"] = self.state["context"]["last_10_actions"][-10:]
            ...
            if os.path.exists(base):
                for item in os.listdir(base):
                    ...
                    if os.path.isdir(item_path):
                        ...
                        if os.path.exists(os.path.join(item_path, "SOUL.md")) or os.path.exists(
                            os.path.join(item_path, "AGENT.md")
                        ):
                            agents.append(item)
