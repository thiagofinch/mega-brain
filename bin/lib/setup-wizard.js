 ...
 *
 * Usage:
 *   npx @thiagofinch/mega-brain@latest setup
...
async function configureApiKeys() {
  ...
  // --- OPENAI_API_KEY (RECOMMENDED — for Whisper transcription only) ---
  console.log(
    `  ${chalk.yellow.bold('RECOMMENDED')} ${chalk.bold('OPENAI_API_KEY')} ${chalk.dim('- Whisper transcription for video/audio')}`
  );
  console.log(chalk.dim('    Only needed if you want to transcribe videos/audio via /ingest.'));
  console.log(chalk.dim('    If you already have .txt/.docx transcripts, you can skip this.'));
  ...
  const openaiAnswer = await inquirer.prompt([
    {
      ...
      validate: (input) => {
        if (input && !input.startsWith('sk-')) return chalk.yellow('OpenAI keys typically start with sk-');
        return true;
      },
    },
  ]);
...
async function generateEnvAndSummary(keys, validationResults, pythonResult, nodeResult, pipResult) {
  ...
  const nextStepsLines = [
    ...
    '',
    chalk.white('  Próximos passos:'),
    chalk.dim('  1. Abra o projeto no Claude Code'),
    chalk.dim('  2. O JARVIS irá se apresentar automaticamente'),
    chalk.dim('  3. Use /ingest para alimentar o sistema'),
  ];

  if (!hasOpenAI) {
    nextStepsLines.push('');
    nextStepsLines.push(chalk.dim('  Nota: sem OPENAI_API_KEY, transcrição de vídeo/áudio'));
    nextStepsLines.push(chalk.dim('  não funcionará. Importe arquivos .txt/.docx diretamente.'));
  }

  console.log(boxen(nextStepsLines.join('\n'), {
