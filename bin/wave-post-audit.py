...
def check_2_agent_creation() -> dict:
    ...
    # Check ecosystem-registry.yaml sync
    import yaml
    index_path = AGENTS / "_registry" / "ecosystem-registry.yaml"
    if index_path.exists():
        index = yaml.safe_load(index_path.read_text(encoding="utf-8")) or {}
        index_slugs = set()
        for agent in index.get("agents", []):
            if isinstance(agent, dict):
                index_slugs.add(agent.get("id", ""))
            ...
            for agent_dir in agents_dir.iterdir():
                if agent_dir.is_dir() and (agent_dir / "AGENT.md").exists():
                    if agent_dir.name not in index_slugs:
                        results["skip_reasons"].append({
                            "slug": agent_dir.name,
                            "reason": "Agent exists on disk but NOT in ecosystem-registry.yaml",
                            "severity": "MEDIUM",
                        })
