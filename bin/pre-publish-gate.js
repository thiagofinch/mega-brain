...
// === STEP 3.5: Verify external framework dirs are NOT in the package ===
const externalFrameworkFiles = packFiles.filter(f => f.includes('.mega-brain/') || f.includes('.mega-brain-core/') || f.includes('.claude/agents/'));
if (externalFrameworkFiles.length > 0) {
  console.error(`${RED}[BLOCKED] External framework content found in package! ${externalFrameworkFiles.length} file(s):${NC}`);
  for (const f of externalFrameworkFiles.slice(0, 10)) {
    console.error(`${RED}  ${f}${NC}`);
  }
  foundIssues += externalFrameworkFiles.length;
}
