...
Tasks:
    ...
    3.3 Update ecosystem-registry.yaml
...
def task_update_agent_index(dry_run: bool = False) -> dict:
    """Regenerate agents/_registry/ecosystem-registry.yaml via bin/refresh-registry.py."""
    log.info("=" * 60)
    log.info("TASK 3.3: Update ecosystem-registry.yaml")
    log.info("=" * 60)
    ...
    registry_path = AGENTS / "_registry" / "ecosystem-registry.yaml"
    refresh_script = PROJECT_ROOT / "bin" / "refresh-registry.py"
    stats = {"agents_found": 0, "new_agents": 0, "updated": 0}

    if not refresh_script.exists():
        log.warning("  bin/refresh-registry.py not found. Skipping registry update.")
        return stats

    if dry_run:
        log.info("  [DRY] Would run: python3 bin/refresh-registry.py")
        # Still count agents in current registry for reporting
        if registry_path.exists():
            data = yaml.safe_load(registry_path.read_text(encoding="utf-8")) or {}
            stats["agents_found"] = len(data.get("agents", []))
        return stats
    ...
    if result.returncode != 0:
        log.error("  refresh-registry.py failed: %s", result.stderr[:500])
        return stats

    # Count updated registry
    if registry_path.exists():
        data = yaml.safe_load(registry_path.read_text(encoding="utf-8")) or {}
        stats["agents_found"] = len(data.get("agents", []))
        stats["updated"] = 1

    log.info(f"  Result: {stats['agents_found']} agents in ecosystem-registry.yaml")
    return stats
