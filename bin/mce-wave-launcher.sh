...
Options:
  ...
  --skip-permissions    Wave 2: pass --dangerously-skip-permissions to claude (opt-in)
...
claude -p SKIP_PERMS_PLACEHOLDER --model ${MODEL} "$(cat <<'PROMPT_EOF'
...
RUNNER_EOF
  ...
  # Replace placeholders (cross-platform, no macOS-only sed -i '')
  local skip_perms_flag=""
  [[ "${SKIP_PERMISSIONS}" -eq 1 ]] && skip_perms_flag="--dangerously-skip-permissions"
  python3 -c "
import pathlib
p = pathlib.Path('${runner}')
txt = p.read_text()
txt = txt.replace('SLUG_PLACEHOLDER', '${slug}')
txt = txt.replace('SKIP_PERMS_PLACEHOLDER', '${skip_perms_flag}')
p.write_text(txt)
"
  chmod +x "${runner}"
  echo "${runner}"
